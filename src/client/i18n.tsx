import { createContext, useCallback, useContext, useEffect, useMemo, useState } from "react";
import type { ReactNode } from "react";

export type Locale = "en" | "zh";

const STORAGE_KEY = "owd_locale";

const dictionaries = {
  en: {
    // Top-level / nav
    "app.brand.name": "Lumynn",
    "app.brand.tagline": "Private Markdown workspace",
    "app.openWorkspace": "Open workspace",
    "nav.aria": "Primary",
    "nav.workspace": "Workspace",
    "nav.indexing": "Indexing",
    "nav.settings": "Settings",
    "topbar.toggleTheme": "Toggle theme",
    "topbar.themeLight": "Light Theme",
    "topbar.themeDark": "Dark Theme",
    "topbar.language": "Language",
    "topbar.logout": "Log Out",
    "topbar.logoutBusy": "Logging Out\u2026",
    "topbar.signedInAs": "Signed in as {name}",
    "topbar.adminBadge": "admin",
    "skipToMain": "Skip to Main Content",

    // Login
    "login.eyebrow": "Lumynn",
    "login.titleSetup": "Create your admin password",
    "login.titleWelcome": "Welcome back",
    "login.description": "Manage a private Markdown library with preview, settings, and RAG Q&A from a modern web interface.",
    "login.username": "Username",
    "login.password": "Password",
    "login.submitSetup": "Create Account",
    "login.submitLogin": "Log In",
    "login.submitSetupBusy": "Creating Account\u2026",
    "login.submitLoginBusy": "Logging In\u2026",

    // Mobile section switcher
    "section.vault": "Library",
    "section.editor": "Editor",
    "section.ask": "Ask",
    "section.aria": "Workspace sections",

    // Vault pane
    "vault.eyebrow": "Library",
    "vault.title": "Documents",
    "vault.collapse": "Collapse library",
    "vault.expand": "Expand library",
    "vault.fileCount": "{count} Markdown file",
    "vault.fileCountPlural": "{count} Markdown files",
    "vault.searchVault": "Search Library",
    "vault.newNote": "New Note",
    "vault.refresh": "Refresh library tree",
    "vault.refreshBusy": "Refreshing library tree",
    "vault.sortBy": "Sort By",
    "vault.sortName": "Name",
    "vault.sortCreated": "Created",
    "vault.sortUpdated": "Updated",
    "vault.sortPath": "Path",
    "vault.sortTitle": "Title",
    "vault.order": "Order",
    "vault.orderAsc": "Ascending",
    "vault.orderDesc": "Descending",
    "vault.empty": "No Markdown documents yet. Create your first note.",
    "vault.sortOptions": "Sort options",
    "vault.search": "Search",
    "vault.new": "New Note",
    "vault.sort": "Sort",
    "vault.done": "Done",

    "quick.trigger": "Quick note",
    "quick.title": "Quick note",
    "quick.draftTitle": "Quick note",
    "quick.draftEyebrow": "Unsaved draft",
    "quick.draftBadge": "Draft",
    "quick.description": "Capture an idea now; the file name is derived from the first heading or line. Saves to {folder}.",
    "quick.placeholder": "Start writing\u2026",
    "quick.save": "Save",
    "quick.saveBusy": "Saving\u2026",
    "quick.cancel": "Cancel",
    "quick.savedToast": "Saved to {path}",
    "quick.timestampPrefix": "Quick note",
    "quick.folderDefault": "Quick notes",
    "vault.pullToSearch": "Pull to search",
    "vault.releaseToSearch": "Release to search",
    "vault.backToTop": "Scroll library list back to top",

    // Editor pane
    "editor.noDocSelected": "No document selected",
    "editor.title": "Editor",
    "editor.modeLabel": "Editor display mode",
    "editor.modeEdit": "Edit",
    "editor.modePreview": "Preview",
    "editor.modeWysiwyg": "WYSIWYG",
    "editor.modeSource": "Source",
    "editor.actionsLabel": "Editor actions",
    "editor.rename": "Rename",
    "editor.move": "Move To\u2026",
    "editor.delete": "Delete",
    "editor.save": "Save Note",
    "editor.saveBusy": "Saving\u2026",
    "editor.tabsLabel": "Open documents",
    "editor.closeTab": "Close {name}",
    "editor.tabMenu.label": "Tab actions for {name}",
    "editor.tabMenu.close": "Close Tab",
    "editor.tabMenu.closeOthers": "Close Other Tabs",
    "editor.tabMenu.closeAll": "Close All Tabs",
    "editor.contentLabel": "Markdown Content",
    "editor.sourceContentLabel": "Raw Markdown source",
    "editor.loading": "Loading editor…",
    "editor.loadFailed": "Unable to load the editor. Reconnect and refresh this page.",
    "editor.previewEmpty": "No preview yet.",
    "editor.blankEyebrow": "No file open",
    "editor.blankTitle": "Select a Document from the Tree",
    "editor.blankBody": "The editor starts blank by default. Open one or more files to work with tabs.",
    "editor.fab.save": "Save",
    "editor.fab.saved": "Saved",
    "editor.fab.saving": "Saving\u2026",
    "editor.fab.edit": "Write",
    "editor.fab.preview": "Preview",
    "editor.fab.ariaSave": "Save note",
    "editor.fab.ariaSaved": "Note is up to date",
    "editor.fab.ariaEdit": "Edit note",
    "editor.fab.ariaPreview": "Switch to preview",
    "editor.swipeClose": "Close",
    "editor.toggleMode": "Toggle Edit / Preview",
    "editor.moreActions": "More Actions",
    "editor.print": "Print\u2026",
    "editor.copyMarkdown": "Copy Markdown",
    "editor.markdownCopied": "Markdown copied to clipboard.",
    "editor.copyError": "Unable to copy Markdown",
    "editor.copied": "Copied",
    "editor.copyFailed": "Failed",
    "editor.focusEnter": "Focus Mode",
    "editor.focusExit": "Exit Focus Mode",

    // Status messages
    "status.ready": "Ready",
    "status.saving": "Saving\u2026",
    "status.saved": "Saved",
    "status.saveFailed": "Save failed",
    "status.creating": "Creating\u2026",
    "status.created": "Created",
    "status.createFailed": "Create failed",
    "status.renaming": "Renaming\u2026",
    "status.renamed": "Renamed",
    "status.renameFailed": "Rename failed",
    "status.deleting": "Deleting\u2026",
    "status.deleted": "Deleted",
    "status.deleteFailed": "Delete failed",
    "status.restoring": "Restoring\u2026",
    "status.restoreFailedPrefix": "Restore failed",
    "status.restoredPrefix": "Restored",
    "status.reopenedPrefix": "Reopened",
    "status.reopenedTabs": "Reopened {count} tabs",
    "status.uploading": "Uploading\u2026",
    "status.uploadFailed": "Upload failed",
    "status.attachmentSaved": "Image saved to attachments",
    "status.printNothing": "Nothing to print yet. Add some content first.",
    "status.copyNothing": "Nothing to copy yet. Add some content first.",
    "offline.offline": "Offline",
    "offline.syncing": "Syncing local changes\u2026",
    "offline.pending": "{count} local change(s) pending",
    "offline.conflicts": "{count} sync conflict(s)",
    "offline.syncNow": "Sync local changes",
    "offline.conflictStatus": "{count} sync conflict(s). Local copies were kept.",
    "offline.pendingStatus": "{count} local change(s) still pending.",
    "offline.offlineStatus": "Offline with {count} local change(s) pending.",
    "offline.syncedStatus": "Local changes synced.",
    "offline.cacheBusy": "Caching offline content\u2026",
    "offline.cacheProgress": "Caching offline content {done}/{total}\u2026",
    "offline.cacheNow": "Update offline cache",
    "offline.cacheUpdated": "Offline cache updated ({count} notes).",
    "offline.pinActive": "Make current note available offline",
    "offline.unpinActive": "Remove current note from offline list",
    "offline.pinFolder": "Make selected folder available offline",
    "offline.unpinFolder": "Remove selected folder from offline list",
    "offline.enableFullLibrary": "Cache full library offline",
    "offline.disableFullLibrary": "Stop full-library offline cache",
    "offline.includedByFullLibrary": "Included by full-library offline cache",
    "offline.pinStarted": "Offline cache started.",
    "offline.pinRemoved": "Removed from offline list.",
    "offline.fullStarted": "Full-library offline cache started.",
    "offline.fullRemoved": "Full-library offline cache disabled.",

    // Undo toast
    "undo.closedPrefix": "Closed",
    "undo.closedTabs": "Closed {count} tabs",
    "undo.deletedPrefix": "Deleted",
    "undo.button": "Undo",
    "undo.dismiss": "Dismiss",

    // Search modal
    "search.eyebrow": "Library Search",
    "search.title": "Search Documents",
    "search.description": "Uses ripgrep for fast library search, with filesystem search as fallback.",
    "search.close": "Close",
    "search.queryLabel": "Search query",
    "search.placeholder": "Search file names, tags, headings, or content\u2026",
    "search.submit": "Search",
    "search.submitBusy": "Searching\u2026",
    "search.empty": "Enter a query to search the library.",
    "search.noResults": "No matching files found.",

    // Prompt / confirm modals
    "prompt.cancel": "Cancel",
    "modal.close": "Close",
    "preview.link.errorTitle": "Unable to Open Link",
    "preview.link.headingMissing": "Heading not found: {target}",
    "preview.link.opened": "Opened {name}",
    "preview.link.openError": "Unable to open link",
    "preview.link.unsupported": "This local link is not a supported note or file: {target}",
    "preview.asset.eyebrow": "File Preview",
    "preview.asset.download": "Download",
    "preview.asset.openNewTab": "Open in New Tab",
    "preview.asset.unsupported": "Preview is not available for this file type.",
    "prompt.create.title": "New Note",
    "prompt.create.eyebrow": "Library",
    "prompt.create.description": "Pick a folder, then type a file name.",
    "prompt.create.label": "File name",
    "prompt.create.placeholder": "Untitled",
    "prompt.create.submit": "Create",
    "prompt.create.submitBusy": "Creating\u2026",
    "prompt.createFolder.title": "New Folder",
    "prompt.createFolder.eyebrow": "Library",
    "prompt.createFolder.description": "Pick a parent folder, then type a name for the new folder.",
    "prompt.createFolder.submit": "Create Folder",
    "prompt.createFolder.submitBusy": "Creating\u2026",
    "prompt.rename.title": "Rename Note",
    "prompt.rename.titleFolder": "Rename Folder",
    "prompt.rename.description": "Type a new file name. To put it in a different folder, use Move To\u2026 instead.",
    "prompt.rename.descriptionFolder": "Type a new folder name. To put it under a different parent folder, use Move To\u2026 instead.",
    "prompt.rename.label": "New name",
    "prompt.rename.submit": "Rename",
    "prompt.rename.submitBusy": "Renaming\u2026",
    "prompt.move.title": "Move Note",
    "prompt.move.titleFolder": "Move Folder",
    "prompt.move.description": "Pick the destination folder for \u201c{name}\u201d. The name stays the same.",
    "prompt.move.submit": "Move Here",
    "prompt.move.submitBusy": "Moving\u2026",
    "prompt.pickFolder": "Destination folder",
    "prompt.targetFolder": "Will be saved into: {folder}",
    "prompt.fileName": "File name",
    "prompt.folderName": "Folder name",
    "prompt.error.emptyName": "Name cannot be empty.",
    "vault.newFolder": "New Folder",
    "folderPicker.vaultRoot": "Library root",
    "folderPicker.loading": "Loading\u2026",
    "folderPicker.collapse": "Collapse folder",
    "folderPicker.expand": "Expand folder",
    "confirm.delete.title": "Delete {name}?",
    "confirm.delete.titleFolder": "Delete folder {name}?",
    "confirm.delete.description": "The file is removed from the library. You can restore it from the Undo toast within a few seconds.",
    "confirm.delete.descriptionFolder": "Empty folders are removed immediately. If the folder still contains notes you'll be asked again to confirm a recursive delete.",
    "confirm.delete.submit": "Delete",
    "confirm.delete.submitBusy": "Deleting\u2026",
    "confirm.deleteFolderRecursive.title": "Delete Folder and Everything Inside?",
    "confirm.deleteFolderRecursive.description": "This will delete the folder and {count} note(s) inside it. This cannot be undone.",
    "confirm.deleteFolderRecursive.submit": "Delete Everything",
    "tree.menu.open": "Open",
    "tree.menu.rename": "Rename\u2026",
    "tree.menu.move": "Move To\u2026",
    "tree.menu.delete": "Delete\u2026",
    "tree.menu.newNoteHere": "New Note Here",
    "tree.menu.newFolderHere": "New Folder Here",
    "tree.menu.copyPath": "Copy Path",
    "tree.menu.offlineMake": "Make Available Offline",
    "tree.menu.offlineRemove": "Remove Offline Availability",
    "tree.menu.offlineIncluded": "Included by Full-Library Offline Cache",
    "tree.menu.copied": "Path copied to clipboard.",
    "tree.menu.copyFailed": "Could not copy path.",
    "tree.dragDrop.busy": "Moving\u2026",
    "tree.dragDrop.refused": "Cannot drop a folder into itself or its descendants.",

    "copilot.eyebrow": "Agent Chat",
    "copilot.title": "Copilot",
    "copilot.collapse": "Collapse Copilot",
    "copilot.expand": "Expand Copilot",
    "copilot.toolbar": "Chat actions",
    "copilot.close": "Close Copilot",
    "copilot.new": "New",
    "copilot.save": "Save",
    "copilot.saveBusy": "Saving\u2026",
    "copilot.load": "Load",
    "copilot.loadBusy": "Loading\u2026",
    "copilot.loadSelect": "Saved chats",
    "copilot.history": "History",
    "copilot.history.empty": "No chat history",
    "copilot.history.open": "Open history file",
    "copilot.history.delete": "Delete history",
    "copilot.answer.actions": "Answer actions",
    "copilot.answer.copy": "Copy Markdown",
    "copilot.answer.copied": "Answer Markdown copied",
    "copilot.answer.copyError": "Unable to copy answer Markdown",
    "copilot.answer.save": "Save Markdown",
    "copilot.answer.saveEyebrow": "Answer Markdown",
    "copilot.answer.saveTitle": "Save answer",
    "copilot.answer.saveDescription": "Choose a library folder and file name for this assistant answer.",
    "copilot.answer.fileName": "File name",
    "copilot.answer.saveSubmit": "Save",
    "copilot.answer.saveBusy": "Saving...",
    "copilot.answer.saveError": "Unable to save answer Markdown",
    "copilot.answer.saveNameError": "Enter a valid file name",
    "copilot.answer.saved": "Saved answer to {path}",
    "copilot.mode.vaultQa": "Library QA",
    "copilot.empty": "Ask a question or request a note edit.",
    "copilot.user": "You",
    "copilot.assistant": "Assistant",
    "copilot.input": "Message",
    "copilot.placeholder": "Ask about notes, use @ to reference notes, or draft an edit\u2026",
    "copilot.disabledPlaceholder": "Configure a Chat Completions provider to use agent chat.",
    "copilot.send": "Send",
    "copilot.stop": "Stop",
    "copilot.status.thinking": "Thinking\u2026",
    "copilot.status.stopped": "Stopped",
    "copilot.status.loaded": "Conversation loaded",
    "copilot.context.label": "Attached note context",
    "copilot.context.current": "Current included",
    "copilot.context.currentOff": "Current excluded",
    "copilot.context.currentShort": "Current",
    "copilot.context.currentOffShort": "Off",
    "copilot.context.attached": "Attached",
    "copilot.context.dirty": "Unsaved",
    "copilot.context.remove": "Remove note context",
    "copilot.notePicker.label": "Attach note",
    "copilot.notePicker.empty": "No matching notes",
    "copilot.notePicker.loading": "Loading notes...",
    "copilot.notePicker.loadError": "Unable to load note list",
    "copilot.notePicker.error": "Unable to load note list: {message}",
    "copilot.notePicker.activeNote": "Active Note",
    "copilot.notePicker.activeBadge": "Current",
    "copilot.notePicker.notes": "Notes",
    "copilot.notePicker.notesSubtitle": "Reference notes in your library",
    "copilot.notePicker.folders": "Folders",
    "copilot.notePicker.foldersSubtitle": "Browse by library folder",
    "copilot.notePicker.categoryBadge": "Category",
    "copilot.notePicker.noteBadge": "Note",
    "copilot.notePicker.folderBadge": "Folder",
    "copilot.notePicker.back": "Back",
    "copilot.notePicker.backSubtitle": "Return to mention categories",
    "copilot.sources": "Sources",
    "copilot.sources.more": "+{count} more source(s)",
    "copilot.sources.moreCompact": "+{count}",
    "copilot.tool.running": "Running",
    "copilot.tool.done": "Done",
    "copilot.proposal.apply": "Apply",
    "copilot.proposal.reject": "Reject",
    "copilot.proposal.rejected": "Rejected",
    "copilot.error.stream": "Unable to run Copilot chat",
    "copilot.error.save": "Unable to save chat",
    "copilot.error.load": "Unable to load chat",
    "copilot.error.delete": "Unable to delete chat history",
    "copilot.error.apply": "Unable to apply edit",

    // Settings (shared)
    "settings.loading": "Loading settings\u2026",
    "settings.eyebrowConfig": "Configuration",
    "settings.eyebrowKb": "Knowledge Base",
    "settings.titleSettings": "Settings",
    "settings.titleIndexing": "Indexing",
    "settings.descriptionSettings": "Configure the library, AI providers, import/export, HTTPS, and the admin account.",
    "settings.descriptionIndexing": "Build, resume, and monitor the searchable RAG index for your Markdown library.",
    "settings.navAria": "Settings sections",

    "settings.section.vault": "Library",
    "settings.section.providers": "AI Providers",
    "settings.section.importExport": "Import / Export",
    "settings.section.https": "HTTPS",
    "settings.section.users": "Users",
    "settings.section.account": "Account",

    // Settings -> Account
    "settings.account.eyebrow": "Authentication",
    "settings.account.title": "Your Account",
    "settings.account.description": "Update the single admin account used by this MVP.",
    "settings.account.descriptionMulti": "Your username and role are managed by an administrator. You can change your password here.",
    "settings.account.username": "Username",
    "settings.account.usernameLocked": "Usernames are stable identifiers and cannot be changed.",
    "settings.account.role": "Role",
    "settings.account.roleAdmin": "Administrator",
    "settings.account.roleUser": "User",
    "settings.account.changePasswordTitle": "Change Password",
    "settings.account.currentPassword": "Current password",
    "settings.account.newPassword": "New password",
    "settings.account.changePassword": "Change Password",
    "settings.account.changePasswordBusy": "Changing\u2026",
    "settings.account.passwordChanged": "Password changed. Other sessions for this account were signed out.",
    "settings.account.passwordChangeError": "Unable to change password",
    "settings.account.save": "Save Account",
    "settings.account.saveBusy": "Saving\u2026",
    "settings.account.savedMessage": "Account credentials saved. Existing sessions were cleared.",
    "settings.account.saveError": "Unable to save account",
    "settings.account.passwordHelp": "At least 8 characters.",

    // Settings -> Users (admin only)
    "settings.users.eyebrow": "User management",
    "settings.users.title": "Users",
    "settings.users.description": "Add and remove users. Each user has an isolated library and AI provider configuration.",
    "settings.users.createTitle": "Add User",
    "settings.users.username": "Username",
    "settings.users.usernamePlaceholder": "alice",
    "settings.users.password": "Initial password",
    "settings.users.create": "Add User",
    "settings.users.createBusy": "Adding\u2026",
    "settings.users.createdMessage": "User \"{name}\" added. Share the password securely.",
    "settings.users.createError": "Unable to add user",
    "settings.users.listTitle": "Existing Users",
    "settings.users.loading": "Loading users\u2026",
    "settings.users.loadError": "Unable to load users",
    "settings.users.empty": "No users yet.",
    "settings.users.roleAdmin": "admin",
    "settings.users.roleUser": "user",
    "settings.users.vault": "Library: {status}",
    "settings.users.vaultConfigured": "configured",
    "settings.users.vaultMissing": "not configured",
    "settings.users.delete": "Delete\u2026",
    "settings.users.confirmDelete": "Confirm Delete",
    "settings.users.cancel": "Cancel",
    "settings.users.deleteBusy": "Deleting\u2026",
    "settings.users.deletedMessage": "User \"{name}\" deleted.",
    "settings.users.deleteError": "Unable to delete user",
    "settings.users.deleteCaveat": "Deleting a user removes their RAG index and metadata cache. Their Markdown library folder on disk is left untouched.",

    // Settings -> Vault
    "settings.vault.eyebrow": "Documents",
    "settings.vault.title": "Lumynn Library",
    "settings.vault.description": "Choose the filesystem library used by Lumynn.",
    "settings.vault.path": "Library path",
    "settings.vault.allowPlain": "Allow plain Markdown folders",
    "settings.vault.save": "Save Library",
    "settings.vault.saveBusy": "Saving\u2026",
    "settings.vault.savedMessage": "Library settings saved",
    "settings.vault.saveError": "Unable to save library",

    // Settings -> HTTPS
    "settings.https.eyebrow": "Transport",
    "settings.https.title": "HTTPS Certificate",
    "settings.https.description": "Paste or import PEM certificate and private key files. A server restart is required after saving.",
    "settings.https.enable": "Enable HTTPS on server restart",
    "settings.https.statusConfigured": "configured",
    "settings.https.statusNot": "not configured",
    "settings.https.statusLine": "Certificate: {cert} \u00b7 Private key: {key}",
    "settings.https.cert": "Certificate PEM",
    "settings.https.certPlaceholder": "Paste -----BEGIN CERTIFICATE----- \u2026 Leave blank to keep the existing certificate.",
    "settings.https.importCert": "Import Certificate File",
    "settings.https.privateKey": "Private key PEM",
    "settings.https.privateKeyPlaceholder": "Paste -----BEGIN PRIVATE KEY----- \u2026 Leave blank to keep the existing private key.",
    "settings.https.importKey": "Import Private Key File",
    "settings.https.save": "Save HTTPS Settings",
    "settings.https.saveBusy": "Saving\u2026",
    "settings.https.savedMessage": "HTTPS settings saved. Restart the server for protocol changes to take effect.",
    "settings.https.saveError": "Unable to save HTTPS settings",

    // Settings -> Providers
    "settings.providers.embeddingEyebrow": "Embeddings",
    "settings.providers.embeddingTitle": "Embedding Provider",
    "settings.providers.qaEyebrow": "Chat",
    "settings.providers.qaTitle": "Q&A Provider",
    "settings.providers.save": "Save AI Providers",
    "settings.providers.saveBusy": "Saving\u2026",
    "settings.providers.savedMessage": "RAG settings saved",
    "settings.providers.saveError": "Unable to save RAG settings",
    "settings.providers.testEmbedding": "Test Embedding",
    "settings.providers.testEmbeddingBusy": "Testing\u2026",
    "settings.providers.testQa": "Test Q&A",
    "settings.providers.testQaBusy": "Testing\u2026",
    "settings.providers.testFailed": "Test failed",
    "settings.providers.provider": "Provider",
    "settings.providers.providerDisabled": "Disabled",
    "settings.providers.providerOpenAi": "OpenAI compatible",
    "settings.providers.apiMode": "API mode",
    "settings.providers.apiModeEmbeddings": "Embeddings API (/embeddings)",
    "settings.providers.apiModeChat": "Chat Completions API (/chat/completions)",
    "settings.providers.apiModeResponses": "Responses API (/responses)",
    "settings.providers.apiModeCustomEmbedding": "Custom embeddings-compatible path",
    "settings.providers.apiModeCustomChat": "Custom chat-completions-compatible path",
    "settings.providers.baseUrl": "Base URL",
    "settings.providers.endpointPath": "Endpoint path",
    "settings.providers.reasoning": "Reasoning mode",
    "settings.providers.reasoningDisabled": "Disable reasoning/thinking",
    "settings.providers.reasoningDefault": "Provider default",
    "settings.providers.reasoningHelp": "Use disabled for reasoning models that may spend output tokens before the final answer.",
    "settings.providers.model": "Model",
    "settings.providers.apiKey": "API key",
    "settings.providers.apiKeyPlaceholder": "Leave blank to keep existing key",

    // Settings -> Operations / Indexing
    "settings.ops.eyebrow": "Search Index",
    "settings.ops.title": "Build Index",
    "settings.ops.description": "Create the knowledge base used by Ask AI. Start with a small sample, then run incremental indexing for day-to-day updates.",
    "settings.ops.retrievalGroup": "Retrieval",
    "settings.ops.indexingGroup": "Indexing",
    "settings.ops.topK": "Top K",
    "settings.ops.chunkSize": "Chunk size",
    "settings.ops.chunkOverlap": "Chunk overlap",
    "settings.ops.batchSize": "Embedding batch size",
    "settings.ops.rpm": "Embedding requests/min",
    "settings.ops.partitions": "Number of partitions",
    "settings.ops.saveIndex": "Save Index Settings",
    "settings.ops.saveIndexBusy": "Saving\u2026",
    "settings.ops.resetIndex": "Reset Index Settings",
    "settings.ops.resetIndexBusy": "Resetting\u2026",
    "settings.ops.resetSaved": "Index settings reset to defaults and saved",
    "settings.ops.resetError": "Unable to reset index settings",
    "settings.ops.testIndex": "Index 20-File Sample",
    "settings.ops.testIndexBusy": "Starting\u2026",
    "settings.ops.incremental": "Start Incremental Index",
    "settings.ops.incrementalBusy": "Starting\u2026",
    "settings.ops.full": "Rebuild Full Index",
    "settings.ops.fullBusy": "Starting\u2026",
    "settings.ops.startedMessage": "Started {mode} indexing job {id}",
    "settings.ops.startError": "Unable to start indexing",
    "settings.ops.stopRequested": "Stop requested for indexing job",
    "settings.ops.skipRequested": "Skip requested for current file",
    "settings.ops.controlError": "Unable to update indexing job",

    // Settings -> Index status / progress
    "settings.indexStatus.aria": "RAG index status",
    "settings.indexStatus.production": "Production Index",
    "settings.indexStatus.test": "Test Index",
    "settings.indexStatus.indexed": "Indexed",
    "settings.indexStatus.notIndexed": "Not Indexed",
    "settings.indexStatus.files": "{count} files",
    "settings.indexStatus.chunks": "{count} chunks",
    "settings.indexStatus.lastUpdated": "Last updated: {value}",
    "settings.indexStatus.never": "Never",

    "settings.progress.test": "Test index",
    "settings.progress.incremental": "Incremental index",
    "settings.progress.full": "Full index",
    "settings.progress.files": "Files",
    "settings.progress.chunks": "Chunks",
    "settings.progress.skippedFiles": "Skipped files: {count}",
    "settings.progress.reusedChunks": "Reused chunks: {count}",
    "settings.progress.failedChunks": "Failed chunks: {count}",
    "settings.progress.currentFile": "Current: {name}",
    "settings.progress.skipCurrent": "Skip current file",
    "settings.progress.skipRequestedShort": "Skip requested",
    "settings.progress.stop": "Stop indexing",
    "settings.progress.stopping": "Stopping\u2026",
    "settings.progress.elapsed": "{ms}\u00a0ms",
    "settings.jobStatus.queued": "Queued",
    "settings.jobStatus.running": "Running",
    "settings.jobStatus.completed": "Completed",
    "settings.jobStatus.cancelled": "Cancelled",
    "settings.jobStatus.failed": "Failed",
    "settings.message.dismiss": "Dismiss message",

    // Settings -> Import / Export
    "settings.export.eyebrow": "Export",
    "settings.export.title": "RAG Export",
    "settings.export.description": "Exported config omits real API keys from normal settings responses.",
    "settings.export.aria": "RAG configuration export",
    "settings.export.copy": "Copy to clipboard",
    "settings.export.copyBusy": "Copying\u2026",
    "settings.export.copySuccess": "RAG configuration copied to clipboard",
    "settings.export.copyError": "Clipboard write failed. Select the export text and copy it manually.",
    "settings.export.toFile": "Export to file",
    "settings.export.fileSavedMessage": "RAG configuration exported to file",

    "settings.import.eyebrow": "Import",
    "settings.import.title": "RAG Import",
    "settings.import.description": "Paste from clipboard or load a schemaVersion 1 RAG config JSON file.",
    "settings.import.aria": "RAG configuration import",
    "settings.import.paste": "Paste from clipboard",
    "settings.import.pasteBusy": "Pasting\u2026",
    "settings.import.pasteSuccess": "RAG configuration pasted from clipboard",
    "settings.import.pasteError": "Clipboard read failed. Paste the configuration manually.",
    "settings.import.fromFile": "Import from file",
    "settings.import.loadedFile": "Loaded {name}. Review it, then click Import RAG config.",
    "settings.import.submit": "Import RAG config",
    "settings.import.submitBusy": "Importing\u2026",
    "settings.import.success": "RAG configuration imported",
    "settings.import.failed": "Import failed",

    // Generic action failures (used in error fallbacks)
    "error.actionFailed": "Action failed"
  },
  zh: {
    "app.brand.name": "Lumynn",
    "app.brand.tagline": "\u79c1\u5bc6 Markdown \u5de5\u4f5c\u533a",
    "app.openWorkspace": "\u6253\u5f00\u5de5\u4f5c\u533a",
    "nav.aria": "\u4e3b\u5bfc\u822a",
    "nav.workspace": "\u5de5\u4f5c\u533a",
    "nav.indexing": "\u7d22\u5f15",
    "nav.settings": "\u8bbe\u7f6e",
    "topbar.toggleTheme": "\u5207\u6362\u4e3b\u9898",
    "topbar.themeLight": "\u6d45\u8272\u4e3b\u9898",
    "topbar.themeDark": "\u6df1\u8272\u4e3b\u9898",
    "topbar.language": "\u8bed\u8a00",
    "topbar.logout": "\u9000\u51fa\u767b\u5f55",
    "topbar.logoutBusy": "\u9000\u51fa\u4e2d\u2026",
    "topbar.signedInAs": "\u5f53\u524d\u767b\u5f55\uff1a{name}",
    "topbar.adminBadge": "\u7ba1\u7406\u5458",
    "skipToMain": "\u8df3\u81f3\u4e3b\u8981\u5185\u5bb9",

    "login.eyebrow": "Lumynn",
    "login.titleSetup": "\u521b\u5efa\u7ba1\u7406\u5458\u5bc6\u7801",
    "login.titleWelcome": "\u6b22\u8fce\u56de\u6765",
    "login.description": "\u4ee5\u73b0\u4ee3\u5316 Web \u754c\u9762\u7ba1\u7406\u79c1\u5bc6 Markdown \u6587\u6863\u5e93\uff0c\u63d0\u4f9b\u9884\u89c8\u3001\u8bbe\u7f6e\u4ee5\u53ca\u68c0\u7d22\u589e\u5f3a\u95ee\u7b54\u3002",
    "login.username": "\u7528\u6237\u540d",
    "login.password": "\u5bc6\u7801",
    "login.submitSetup": "\u521b\u5efa\u8d26\u6237",
    "login.submitLogin": "\u767b\u5f55",
    "login.submitSetupBusy": "\u521b\u5efa\u4e2d\u2026",
    "login.submitLoginBusy": "\u767b\u5f55\u4e2d\u2026",

    "section.vault": "\u6587\u6863\u5e93",
    "section.editor": "\u7f16\u8f91\u5668",
    "section.ask": "\u63d0\u95ee",
    "section.aria": "\u5de5\u4f5c\u533a\u533a\u57df",

    "vault.eyebrow": "\u6587\u6863\u5e93",
    "vault.title": "\u6587\u6863",
    "vault.collapse": "\u6536\u8d77\u6587\u6863\u5e93",
    "vault.expand": "\u5c55\u5f00\u6587\u6863\u5e93",
    "vault.fileCount": "\u5171 {count} \u4e2a Markdown \u6587\u4ef6",
    "vault.fileCountPlural": "\u5171 {count} \u4e2a Markdown \u6587\u4ef6",
    "vault.searchVault": "\u641c\u7d22\u6587\u6863\u5e93",
    "vault.newNote": "\u65b0\u5efa\u7b14\u8bb0",
    "vault.refresh": "\u5237\u65b0\u6587\u6863\u5e93\u6811",
    "vault.refreshBusy": "\u6b63\u5728\u5237\u65b0\u6587\u6863\u5e93\u6811",
    "vault.sortBy": "\u6392\u5e8f",
    "vault.sortName": "\u540d\u79f0",
    "vault.sortCreated": "\u521b\u5efa\u65f6\u95f4",
    "vault.sortUpdated": "\u4fee\u6539\u65f6\u95f4",
    "vault.sortPath": "\u8def\u5f84",
    "vault.sortTitle": "\u6807\u9898",
    "vault.order": "\u987a\u5e8f",
    "vault.orderAsc": "\u5347\u5e8f",
    "vault.orderDesc": "\u964d\u5e8f",
    "vault.empty": "\u8fd8\u6ca1\u6709 Markdown \u6587\u6863\uff0c\u5148\u521b\u5efa\u4e00\u7bc7\u5427\u3002",
    "vault.sortOptions": "\u6392\u5e8f\u9009\u9879",
    "vault.search": "\u641c\u7d22",
    "vault.new": "\u65b0\u5efa",
    "vault.sort": "\u6392\u5e8f",
    "vault.done": "\u5b8c\u6210",

    "quick.trigger": "\u5feb\u8bb0",
    "quick.title": "\u5feb\u8bb0",
    "quick.draftTitle": "\u5feb\u8bb0",
    "quick.draftEyebrow": "\u672a\u4fdd\u5b58\u8349\u7a3f",
    "quick.draftBadge": "\u8349\u7a3f",
    "quick.description": "\u968f\u624b\u8bb0\u5f55\uff0c\u6587\u4ef6\u540d\u4ece\u9996\u4e2a\u6807\u9898\u6216\u9996\u884c\u81ea\u52a8\u8bc6\u522b\uff0c\u4fdd\u5b58\u5230 {folder}\u3002",
    "quick.placeholder": "\u5f00\u59cb\u8f93\u5165\u2026",
    "quick.save": "\u4fdd\u5b58",
    "quick.saveBusy": "\u4fdd\u5b58\u4e2d\u2026",
    "quick.cancel": "\u53d6\u6d88",
    "quick.savedToast": "\u5df2\u4fdd\u5b58\u81f3 {path}",
    "quick.timestampPrefix": "\u5feb\u8bb0",
    "quick.folderDefault": "\u5feb\u8bb0",
    "vault.pullToSearch": "\u4e0b\u62c9\u641c\u7d22",
    "vault.releaseToSearch": "\u677e\u5f00\u5373\u53ef\u641c\u7d22",
    "vault.backToTop": "\u56de\u5230\u9876\u90e8",

    "editor.noDocSelected": "\u672a\u9009\u4e2d\u6587\u6863",
    "editor.title": "\u7f16\u8f91\u5668",
    "editor.modeLabel": "\u7f16\u8f91\u5668\u663e\u793a\u6a21\u5f0f",
    "editor.modeEdit": "\u7f16\u8f91",
    "editor.modePreview": "\u9884\u89c8",
    "editor.modeWysiwyg": "\u6240\u89c1\u5373\u6240\u5f97",
    "editor.modeSource": "\u6e90\u7801",
    "editor.actionsLabel": "\u7f16\u8f91\u5668\u64cd\u4f5c",
    "editor.rename": "\u91cd\u547d\u540d",
    "editor.move": "\u79fb\u52a8\u5230\u2026",
    "editor.delete": "\u5220\u9664",
    "editor.save": "\u4fdd\u5b58",
    "editor.saveBusy": "\u4fdd\u5b58\u4e2d\u2026",
    "editor.tabsLabel": "\u5df2\u6253\u5f00\u7684\u6587\u6863",
    "editor.closeTab": "\u5173\u95ed {name}",
    "editor.tabMenu.label": "{name} \u7684\u6807\u7b7e\u64cd\u4f5c",
    "editor.tabMenu.close": "\u5173\u95ed\u6807\u7b7e",
    "editor.tabMenu.closeOthers": "\u5173\u95ed\u5176\u4ed6\u6807\u7b7e",
    "editor.tabMenu.closeAll": "\u5173\u95ed\u5168\u90e8\u6807\u7b7e",
    "editor.contentLabel": "Markdown \u5185\u5bb9",
    "editor.sourceContentLabel": "\u539f\u59cb Markdown \u6e90\u7801",
    "editor.loading": "\u7f16\u8f91\u5668\u52a0\u8f7d\u4e2d\u2026",
    "editor.loadFailed": "\u65e0\u6cd5\u52a0\u8f7d\u7f16\u8f91\u5668\uff0c\u8bf7\u6062\u590d\u8fde\u63a5\u540e\u5237\u65b0\u9875\u9762\u3002",
    "editor.previewEmpty": "\u6682\u65e0\u9884\u89c8\u3002",
    "editor.blankEyebrow": "\u672a\u6253\u5f00\u6587\u4ef6",
    "editor.blankTitle": "\u4ece\u5de6\u4fa7\u9009\u62e9\u4e00\u4efd\u6587\u6863",
    "editor.blankBody": "\u7f16\u8f91\u5668\u9ed8\u8ba4\u4e3a\u7a7a\u3002\u6253\u5f00\u4e00\u4e2a\u6216\u591a\u4e2a\u6587\u4ef6\u540e\u4f1a\u4ee5\u6807\u7b7e\u9875\u5bfc\u822a\u3002",
    "editor.fab.save": "\u4fdd\u5b58",
    "editor.fab.saved": "\u5df2\u4fdd\u5b58",
    "editor.fab.saving": "\u4fdd\u5b58\u4e2d\u2026",
    "editor.fab.edit": "\u5199\u4f5c",
    "editor.fab.preview": "\u9884\u89c8",
    "editor.fab.ariaSave": "\u4fdd\u5b58\u5f53\u524d\u6587\u6863",
    "editor.fab.ariaSaved": "\u5f53\u524d\u6587\u6863\u5df2\u662f\u6700\u65b0",
    "editor.fab.ariaEdit": "\u7f16\u8f91\u5f53\u524d\u6587\u6863",
    "editor.fab.ariaPreview": "\u5207\u6362\u5230\u9884\u89c8",
    "editor.swipeClose": "\u5173\u95ed",
    "editor.toggleMode": "\u5207\u6362\u7f16\u8f91 / \u9884\u89c8",
    "editor.moreActions": "\u66f4\u591a\u64cd\u4f5c",
    "editor.print": "\u6253\u5370\u2026",
    "editor.copyMarkdown": "\u590d\u5236 Markdown",
    "editor.markdownCopied": "Markdown \u5df2\u590d\u5236\u5230\u526a\u8d34\u677f\u3002",
    "editor.copyError": "\u65e0\u6cd5\u590d\u5236 Markdown",
    "editor.copied": "\u5df2\u590d\u5236",
    "editor.copyFailed": "\u5931\u8d25",
    "editor.focusEnter": "\u4e13\u6ce8\u6a21\u5f0f",
    "editor.focusExit": "\u9000\u51fa\u4e13\u6ce8\u6a21\u5f0f",

    "status.ready": "\u5c31\u7eea",
    "status.saving": "\u4fdd\u5b58\u4e2d\u2026",
    "status.saved": "\u5df2\u4fdd\u5b58",
    "status.saveFailed": "\u4fdd\u5b58\u5931\u8d25",
    "status.creating": "\u521b\u5efa\u4e2d\u2026",
    "status.created": "\u5df2\u521b\u5efa",
    "status.createFailed": "\u521b\u5efa\u5931\u8d25",
    "status.renaming": "\u91cd\u547d\u540d\u4e2d\u2026",
    "status.renamed": "\u5df2\u91cd\u547d\u540d",
    "status.renameFailed": "\u91cd\u547d\u540d\u5931\u8d25",
    "status.deleting": "\u5220\u9664\u4e2d\u2026",
    "status.deleted": "\u5df2\u5220\u9664",
    "status.deleteFailed": "\u5220\u9664\u5931\u8d25",
    "status.restoring": "\u6062\u590d\u4e2d\u2026",
    "status.restoreFailedPrefix": "\u6062\u590d\u5931\u8d25",
    "status.restoredPrefix": "\u5df2\u6062\u590d",
    "status.reopenedPrefix": "\u5df2\u91cd\u65b0\u6253\u5f00",
    "status.reopenedTabs": "\u5df2\u91cd\u65b0\u6253\u5f00 {count} \u4e2a\u6807\u7b7e",
    "status.uploading": "\u4e0a\u4f20\u4e2d\u2026",
    "status.uploadFailed": "\u4e0a\u4f20\u5931\u8d25",
    "status.attachmentSaved": "\u56fe\u7247\u5df2\u4fdd\u5b58\u5230 attachments",
    "status.printNothing": "\u8fd8\u6ca1\u6709\u53ef\u6253\u5370\u7684\u5185\u5bb9\uff0c\u5148\u5199\u70b9\u4ec0\u4e48\u5427\u3002",
    "status.copyNothing": "\u8fd8\u6ca1\u6709\u53ef\u590d\u5236\u7684\u5185\u5bb9\uff0c\u5148\u5199\u70b9\u4ec0\u4e48\u5427\u3002",
    "offline.offline": "\u79bb\u7ebf",
    "offline.syncing": "\u672c\u5730\u4fee\u6539\u540c\u6b65\u4e2d\u2026",
    "offline.pending": "{count} \u4e2a\u672c\u5730\u4fee\u6539\u5f85\u540c\u6b65",
    "offline.conflicts": "{count} \u4e2a\u540c\u6b65\u51b2\u7a81",
    "offline.syncNow": "\u540c\u6b65\u672c\u5730\u4fee\u6539",
    "offline.conflictStatus": "\u53d1\u73b0 {count} \u4e2a\u540c\u6b65\u51b2\u7a81\uff0c\u672c\u5730\u526f\u672c\u5df2\u4fdd\u7559\u3002",
    "offline.pendingStatus": "\u4ecd\u6709 {count} \u4e2a\u672c\u5730\u4fee\u6539\u5f85\u540c\u6b65\u3002",
    "offline.offlineStatus": "\u79bb\u7ebf\u4e2d\uff0c{count} \u4e2a\u672c\u5730\u4fee\u6539\u5f85\u540c\u6b65\u3002",
    "offline.syncedStatus": "\u672c\u5730\u4fee\u6539\u5df2\u540c\u6b65\u3002",
    "offline.cacheBusy": "\u79bb\u7ebf\u5185\u5bb9\u7f13\u5b58\u4e2d\u2026",
    "offline.cacheProgress": "\u79bb\u7ebf\u5185\u5bb9\u7f13\u5b58\u4e2d {done}/{total}\u2026",
    "offline.cacheNow": "\u66f4\u65b0\u79bb\u7ebf\u7f13\u5b58",
    "offline.cacheUpdated": "\u79bb\u7ebf\u7f13\u5b58\u5df2\u66f4\u65b0\uff08{count} \u7bc7\u7b14\u8bb0\uff09\u3002",
    "offline.pinActive": "\u5c06\u5f53\u524d\u7b14\u8bb0\u8bbe\u4e3a\u79bb\u7ebf\u53ef\u7528",
    "offline.unpinActive": "\u4ece\u79bb\u7ebf\u5217\u8868\u79fb\u9664\u5f53\u524d\u7b14\u8bb0",
    "offline.pinFolder": "\u5c06\u9009\u4e2d\u6587\u4ef6\u5939\u8bbe\u4e3a\u79bb\u7ebf\u53ef\u7528",
    "offline.unpinFolder": "\u4ece\u79bb\u7ebf\u5217\u8868\u79fb\u9664\u9009\u4e2d\u6587\u4ef6\u5939",
    "offline.enableFullLibrary": "\u6574\u4e2a\u6587\u6863\u5e93\u79bb\u7ebf\u7f13\u5b58",
    "offline.disableFullLibrary": "\u505c\u6b62\u6574\u5e93\u79bb\u7ebf\u7f13\u5b58",
    "offline.includedByFullLibrary": "\u5df2\u5305\u542b\u5728\u6574\u5e93\u79bb\u7ebf\u7f13\u5b58\u4e2d",
    "offline.pinStarted": "\u5df2\u5f00\u59cb\u7f13\u5b58\u79bb\u7ebf\u5185\u5bb9\u3002",
    "offline.pinRemoved": "\u5df2\u4ece\u79bb\u7ebf\u5217\u8868\u79fb\u9664\u3002",
    "offline.fullStarted": "\u5df2\u5f00\u59cb\u6574\u5e93\u79bb\u7ebf\u7f13\u5b58\u3002",
    "offline.fullRemoved": "\u5df2\u5173\u95ed\u6574\u5e93\u79bb\u7ebf\u7f13\u5b58\u3002",

    "undo.closedPrefix": "\u5df2\u5173\u95ed",
    "undo.closedTabs": "\u5df2\u5173\u95ed {count} \u4e2a\u6807\u7b7e",
    "undo.deletedPrefix": "\u5df2\u5220\u9664",
    "undo.button": "\u64a4\u9500",
    "undo.dismiss": "\u5173\u95ed\u63d0\u793a",

    "search.eyebrow": "\u6587\u6863\u5e93\u641c\u7d22",
    "search.title": "\u641c\u7d22\u6587\u6863",
    "search.description": "\u4f18\u5148\u4f7f\u7528 ripgrep \u5feb\u901f\u641c\u7d22\uff0c\u672a\u53ef\u7528\u65f6\u56de\u9000\u5230\u6587\u4ef6\u7cfb\u7edf\u641c\u7d22\u3002",
    "search.close": "\u5173\u95ed",
    "search.queryLabel": "\u641c\u7d22\u5173\u952e\u5b57",
    "search.placeholder": "\u641c\u7d22\u6587\u4ef6\u540d\u3001\u6807\u7b7e\u3001\u6807\u9898\u6216\u6b63\u6587\u2026",
    "search.submit": "\u641c\u7d22",
    "search.submitBusy": "\u641c\u7d22\u4e2d\u2026",
    "search.empty": "\u8f93\u5165\u5173\u952e\u5b57\u4ee5\u641c\u7d22\u6587\u6863\u5e93\u3002",
    "search.noResults": "\u672a\u627e\u5230\u5339\u914d\u7684\u6587\u4ef6\u3002",

    "prompt.cancel": "\u53d6\u6d88",
    "modal.close": "\u5173\u95ed",
    "preview.link.errorTitle": "\u65e0\u6cd5\u6253\u5f00\u94fe\u63a5",
    "preview.link.headingMissing": "\u672a\u627e\u5230\u6807\u9898\uff1a{target}",
    "preview.link.opened": "\u5df2\u6253\u5f00 {name}",
    "preview.link.openError": "\u65e0\u6cd5\u6253\u5f00\u94fe\u63a5",
    "preview.link.unsupported": "\u8fd9\u4e2a\u672c\u5730\u94fe\u63a5\u4e0d\u662f\u53ef\u652f\u6301\u7684\u7b14\u8bb0\u6216\u6587\u4ef6\uff1a{target}",
    "preview.asset.eyebrow": "\u6587\u4ef6\u9884\u89c8",
    "preview.asset.download": "\u4e0b\u8f7d",
    "preview.asset.openNewTab": "\u5728\u65b0\u6807\u7b7e\u6253\u5f00",
    "preview.asset.unsupported": "\u8fd9\u79cd\u6587\u4ef6\u7c7b\u578b\u65e0\u6cd5\u9884\u89c8\u3002",
    "prompt.create.title": "\u65b0\u5efa\u7b14\u8bb0",
    "prompt.create.eyebrow": "\u6587\u6863\u5e93",
    "prompt.create.description": "\u9009\u62e9\u4e00\u4e2a\u6587\u4ef6\u5939\uff0c\u7136\u540e\u8f93\u5165\u6587\u4ef6\u540d\u3002",
    "prompt.create.label": "\u6587\u4ef6\u540d",
    "prompt.create.placeholder": "Untitled",
    "prompt.create.submit": "\u521b\u5efa",
    "prompt.create.submitBusy": "\u521b\u5efa\u4e2d\u2026",
    "prompt.createFolder.title": "\u65b0\u5efa\u6587\u4ef6\u5939",
    "prompt.createFolder.eyebrow": "\u6587\u6863\u5e93",
    "prompt.createFolder.description": "\u9009\u62e9\u4e00\u4e2a\u7236\u6587\u4ef6\u5939\uff0c\u7136\u540e\u8f93\u5165\u65b0\u6587\u4ef6\u5939\u7684\u540d\u79f0\u3002",
    "prompt.createFolder.submit": "\u521b\u5efa\u6587\u4ef6\u5939",
    "prompt.createFolder.submitBusy": "\u521b\u5efa\u4e2d\u2026",
    "prompt.rename.title": "\u91cd\u547d\u540d\u7b14\u8bb0",
    "prompt.rename.titleFolder": "\u91cd\u547d\u540d\u6587\u4ef6\u5939",
    "prompt.rename.description": "\u8f93\u5165\u65b0\u6587\u4ef6\u540d\u3002\u8981\u79fb\u52a8\u5230\u5176\u4ed6\u6587\u4ef6\u5939\uff0c\u8bf7\u4f7f\u7528\u201c\u79fb\u52a8\u5230\u2026\u201d\u3002",
    "prompt.rename.descriptionFolder": "\u8f93\u5165\u65b0\u6587\u4ef6\u5939\u540d\u3002\u8981\u79fb\u52a8\u5230\u5176\u4ed6\u7236\u6587\u4ef6\u5939\uff0c\u8bf7\u4f7f\u7528\u201c\u79fb\u52a8\u5230\u2026\u201d\u3002",
    "prompt.rename.label": "\u65b0\u540d\u79f0",
    "prompt.rename.submit": "\u91cd\u547d\u540d",
    "prompt.rename.submitBusy": "\u91cd\u547d\u540d\u4e2d\u2026",
    "prompt.move.title": "\u79fb\u52a8\u7b14\u8bb0",
    "prompt.move.titleFolder": "\u79fb\u52a8\u6587\u4ef6\u5939",
    "prompt.move.description": "\u9009\u62e9\u201c{name}\u201d\u7684\u76ee\u6807\u6587\u4ef6\u5939\u3002\u540d\u79f0\u4fdd\u6301\u4e0d\u53d8\u3002",
    "prompt.move.submit": "\u79fb\u52a8\u5230\u8fd9\u91cc",
    "prompt.move.submitBusy": "\u79fb\u52a8\u4e2d\u2026",
    "prompt.pickFolder": "\u76ee\u6807\u6587\u4ef6\u5939",
    "prompt.targetFolder": "\u5c06\u4fdd\u5b58\u5230\uff1a{folder}",
    "prompt.fileName": "\u6587\u4ef6\u540d",
    "prompt.folderName": "\u6587\u4ef6\u5939\u540d",
    "prompt.error.emptyName": "\u540d\u79f0\u4e0d\u80fd\u4e3a\u7a7a\u3002",
    "vault.newFolder": "\u65b0\u5efa\u6587\u4ef6\u5939",
    "folderPicker.vaultRoot": "\u6587\u6863\u5e93\u6839\u76ee\u5f55",
    "folderPicker.loading": "\u52a0\u8f7d\u4e2d\u2026",
    "folderPicker.collapse": "\u6536\u8d77\u6587\u4ef6\u5939",
    "folderPicker.expand": "\u5c55\u5f00\u6587\u4ef6\u5939",
    "confirm.delete.title": "\u5220\u9664 {name}\uff1f",
    "confirm.delete.titleFolder": "\u5220\u9664\u6587\u4ef6\u5939 {name}\uff1f",
    "confirm.delete.description": "\u6587\u4ef6\u5c06\u4ece\u6587\u6863\u5e93\u4e2d\u79fb\u9664\u3002\u51e0\u79d2\u5185\u53ef\u4ece\u201c\u64a4\u9500\u201d\u63d0\u793a\u4e2d\u6062\u590d\u3002",
    "confirm.delete.descriptionFolder": "\u7a7a\u6587\u4ef6\u5939\u4f1a\u88ab\u7acb\u5373\u5220\u9664\u3002\u5982\u679c\u6587\u4ef6\u5939\u4ecd\u5305\u542b\u7b14\u8bb0\uff0c\u4f1a\u8981\u6c42\u4f60\u518d\u6b21\u786e\u8ba4\u9012\u5f52\u5220\u9664\u3002",
    "confirm.delete.submit": "\u5220\u9664",
    "confirm.delete.submitBusy": "\u5220\u9664\u4e2d\u2026",
    "confirm.deleteFolderRecursive.title": "\u5220\u9664\u6587\u4ef6\u5939\u53ca\u5176\u5168\u90e8\u5185\u5bb9\uff1f",
    "confirm.deleteFolderRecursive.description": "\u8fd9\u5c06\u5220\u9664\u8be5\u6587\u4ef6\u5939\u53ca\u5176\u4e2d\u7684 {count} \u4efd\u7b14\u8bb0\u3002\u6b64\u64cd\u4f5c\u4e0d\u53ef\u64a4\u9500\u3002",
    "confirm.deleteFolderRecursive.submit": "\u5168\u90e8\u5220\u9664",
    "tree.menu.open": "\u6253\u5f00",
    "tree.menu.rename": "\u91cd\u547d\u540d\u2026",
    "tree.menu.move": "\u79fb\u52a8\u5230\u2026",
    "tree.menu.delete": "\u5220\u9664\u2026",
    "tree.menu.newNoteHere": "\u5728\u8fd9\u91cc\u65b0\u5efa\u7b14\u8bb0",
    "tree.menu.newFolderHere": "\u5728\u8fd9\u91cc\u65b0\u5efa\u6587\u4ef6\u5939",
    "tree.menu.copyPath": "\u590d\u5236\u8def\u5f84",
    "tree.menu.offlineMake": "\u8bbe\u4e3a\u79bb\u7ebf\u53ef\u7528",
    "tree.menu.offlineRemove": "\u53d6\u6d88\u79bb\u7ebf\u53ef\u7528",
    "tree.menu.offlineIncluded": "\u5df2\u7531\u6574\u5e93\u79bb\u7ebf\u7f13\u5b58\u5305\u542b",
    "tree.menu.copied": "\u8def\u5f84\u5df2\u590d\u5236\u5230\u526a\u8d34\u677f\u3002",
    "tree.menu.copyFailed": "\u590d\u5236\u8def\u5f84\u5931\u8d25\u3002",
    "tree.dragDrop.busy": "\u79fb\u52a8\u4e2d\u2026",
    "tree.dragDrop.refused": "\u4e0d\u80fd\u5c06\u6587\u4ef6\u5939\u62d6\u5165\u81ea\u8eab\u6216\u5176\u5b50\u76ee\u5f55\u3002",

    "copilot.eyebrow": "\u667a\u80fd\u4f53\u804a\u5929",
    "copilot.title": "Copilot",
    "copilot.collapse": "\u6536\u8d77 Copilot",
    "copilot.expand": "\u5c55\u5f00 Copilot",
    "copilot.toolbar": "\u804a\u5929\u64cd\u4f5c",
    "copilot.close": "\u5173\u95ed Copilot",
    "copilot.new": "\u65b0\u5efa",
    "copilot.save": "\u4fdd\u5b58",
    "copilot.saveBusy": "\u4fdd\u5b58\u4e2d\u2026",
    "copilot.load": "\u52a0\u8f7d",
    "copilot.loadBusy": "\u52a0\u8f7d\u4e2d\u2026",
    "copilot.loadSelect": "\u5df2\u4fdd\u5b58\u804a\u5929",
    "copilot.history": "\u5386\u53f2",
    "copilot.history.empty": "\u6682\u65e0\u5386\u53f2",
    "copilot.history.open": "\u5728\u7f16\u8f91\u5668\u6253\u5f00\u5386\u53f2\u6587\u4ef6",
    "copilot.history.delete": "\u5220\u9664\u5386\u53f2",
    "copilot.answer.actions": "\u56de\u7b54\u64cd\u4f5c",
    "copilot.answer.copy": "\u590d\u5236 Markdown",
    "copilot.answer.copied": "\u5df2\u590d\u5236\u56de\u7b54 Markdown",
    "copilot.answer.copyError": "\u65e0\u6cd5\u590d\u5236\u56de\u7b54 Markdown",
    "copilot.answer.save": "\u4fdd\u5b58 Markdown",
    "copilot.answer.saveEyebrow": "\u56de\u7b54 Markdown",
    "copilot.answer.saveTitle": "\u4fdd\u5b58\u56de\u7b54",
    "copilot.answer.saveDescription": "\u9009\u62e9\u6587\u6863\u5e93\u76ee\u5f55\u548c\u6587\u4ef6\u540d\uff0c\u4fdd\u5b58\u8fd9\u6761\u52a9\u624b\u56de\u7b54\u3002",
    "copilot.answer.fileName": "\u6587\u4ef6\u540d",
    "copilot.answer.saveSubmit": "\u4fdd\u5b58",
    "copilot.answer.saveBusy": "\u4fdd\u5b58\u4e2d...",
    "copilot.answer.saveError": "\u65e0\u6cd5\u4fdd\u5b58\u56de\u7b54 Markdown",
    "copilot.answer.saveNameError": "\u8bf7\u8f93\u5165\u6709\u6548\u6587\u4ef6\u540d",
    "copilot.answer.saved": "\u5df2\u4fdd\u5b58\u56de\u7b54\u5230 {path}",
    "copilot.mode.vaultQa": "\u6587\u6863\u5e93\u95ee\u7b54",
    "copilot.empty": "\u63d0\u95ee\u3001\u641c\u7d22\u7b14\u8bb0\uff0c\u6216\u8bf7\u6c42\u7b14\u8bb0\u7f16\u8f91\u3002",
    "copilot.user": "\u4f60",
    "copilot.assistant": "\u52a9\u624b",
    "copilot.input": "\u6d88\u606f",
    "copilot.placeholder": "\u8be2\u95ee\u7b14\u8bb0\uff0c\u7528 @ \u5f15\u7528\u7b14\u8bb0\uff0c\u6216\u8d77\u8349\u7f16\u8f91\u2026",
    "copilot.disabledPlaceholder": "\u8bf7\u914d\u7f6e Chat Completions \u63d0\u4f9b\u65b9\u4ee5\u542f\u7528\u667a\u80fd\u4f53\u804a\u5929\u3002",
    "copilot.send": "\u53d1\u9001",
    "copilot.stop": "\u505c\u6b62",
    "copilot.status.thinking": "\u601d\u8003\u4e2d\u2026",
    "copilot.status.stopped": "\u5df2\u505c\u6b62",
    "copilot.status.loaded": "\u5df2\u52a0\u8f7d\u5bf9\u8bdd",
    "copilot.context.label": "\u5df2\u9644\u52a0\u7b14\u8bb0\u4e0a\u4e0b\u6587",
    "copilot.context.current": "\u5df2\u52a0\u5165\u5f53\u524d",
    "copilot.context.currentOff": "\u672a\u52a0\u5165\u5f53\u524d",
    "copilot.context.currentShort": "\u5f53\u524d",
    "copilot.context.currentOffShort": "\u672a\u52a0\u5165",
    "copilot.context.attached": "\u5df2\u5f15\u7528",
    "copilot.context.dirty": "\u672a\u4fdd\u5b58",
    "copilot.context.remove": "\u79fb\u9664\u7b14\u8bb0\u4e0a\u4e0b\u6587",
    "copilot.notePicker.label": "\u5f15\u7528\u7b14\u8bb0",
    "copilot.notePicker.empty": "\u6ca1\u6709\u5339\u914d\u7684\u7b14\u8bb0",
    "copilot.notePicker.loading": "\u6b63\u5728\u52a0\u8f7d\u7b14\u8bb0\u2026",
    "copilot.notePicker.loadError": "\u65e0\u6cd5\u52a0\u8f7d\u7b14\u8bb0\u5217\u8868",
    "copilot.notePicker.error": "\u65e0\u6cd5\u52a0\u8f7d\u7b14\u8bb0\u5217\u8868\uff1a{message}",
    "copilot.notePicker.activeNote": "\u5f53\u524d\u7b14\u8bb0",
    "copilot.notePicker.activeBadge": "\u5f53\u524d",
    "copilot.notePicker.notes": "\u7b14\u8bb0",
    "copilot.notePicker.notesSubtitle": "\u5f15\u7528\u6587\u6863\u5e93\u4e2d\u7684\u7b14\u8bb0",
    "copilot.notePicker.folders": "\u6587\u4ef6\u5939",
    "copilot.notePicker.foldersSubtitle": "\u6309\u6587\u6863\u5e93\u8def\u5f84\u6d4f\u89c8",
    "copilot.notePicker.categoryBadge": "\u5206\u7c7b",
    "copilot.notePicker.noteBadge": "\u7b14\u8bb0",
    "copilot.notePicker.folderBadge": "\u6587\u4ef6\u5939",
    "copilot.notePicker.back": "\u8fd4\u56de",
    "copilot.notePicker.backSubtitle": "\u56de\u5230\u5f15\u7528\u5206\u7c7b",
    "copilot.sources": "\u6765\u6e90",
    "copilot.sources.more": "\u8fd8\u6709 {count} \u4e2a\u6765\u6e90",
    "copilot.sources.moreCompact": "+{count}",
    "copilot.tool.running": "\u8fd0\u884c\u4e2d",
    "copilot.tool.done": "\u5df2\u5b8c\u6210",
    "copilot.proposal.apply": "\u5e94\u7528",
    "copilot.proposal.reject": "\u62d2\u7edd",
    "copilot.proposal.rejected": "\u5df2\u62d2\u7edd",
    "copilot.error.stream": "\u65e0\u6cd5\u8fd0\u884c Copilot \u804a\u5929",
    "copilot.error.save": "\u65e0\u6cd5\u4fdd\u5b58\u804a\u5929",
    "copilot.error.load": "\u65e0\u6cd5\u52a0\u8f7d\u804a\u5929",
    "copilot.error.delete": "\u65e0\u6cd5\u5220\u9664\u804a\u5929\u5386\u53f2",
    "copilot.error.apply": "\u65e0\u6cd5\u5e94\u7528\u7f16\u8f91",

    "settings.loading": "\u52a0\u8f7d\u8bbe\u7f6e\u4e2d\u2026",
    "settings.eyebrowConfig": "\u914d\u7f6e",
    "settings.eyebrowKb": "\u6587\u6863\u5e93",
    "settings.titleSettings": "\u8bbe\u7f6e",
    "settings.titleIndexing": "\u7d22\u5f15",
    "settings.descriptionSettings": "\u914d\u7f6e\u6587\u6863\u5e93\u3001AI \u63d0\u4f9b\u65b9\u3001\u5bfc\u5165 / \u5bfc\u51fa\u3001HTTPS \u4ee5\u53ca\u7ba1\u7406\u5458\u8d26\u6237\u3002",
    "settings.descriptionIndexing": "\u4e3a Markdown \u6587\u6863\u5e93\u6784\u5efa\u3001\u6062\u590d\u5e76\u76d1\u63a7 RAG \u68c0\u7d22\u7d22\u5f15\u3002",
    "settings.navAria": "\u8bbe\u7f6e\u533a\u57df",

    "settings.section.vault": "\u6587\u6863\u5e93",
    "settings.section.providers": "AI \u63d0\u4f9b\u65b9",
    "settings.section.importExport": "\u5bfc\u5165 / \u5bfc\u51fa",
    "settings.section.https": "HTTPS",
    "settings.section.users": "\u7528\u6237",
    "settings.section.account": "\u8d26\u6237",

    "settings.account.eyebrow": "\u8eab\u4efd\u9a8c\u8bc1",
    "settings.account.title": "\u6211\u7684\u8d26\u6237",
    "settings.account.description": "\u66f4\u65b0\u672c\u9879\u76ee\u4f7f\u7528\u7684\u552f\u4e00\u7ba1\u7406\u5458\u8d26\u6237\u3002",
    "settings.account.descriptionMulti": "\u7528\u6237\u540d\u548c\u89d2\u8272\u7531\u7ba1\u7406\u5458\u7ef4\u62a4\uff0c\u4f60\u53ef\u4ee5\u5728\u8fd9\u91cc\u4fee\u6539\u4f60\u7684\u5bc6\u7801\u3002",
    "settings.account.username": "\u7528\u6237\u540d",
    "settings.account.usernameLocked": "\u7528\u6237\u540d\u662f\u552f\u4e00\u6807\u8bc6\uff0c\u521b\u5efa\u540e\u4e0d\u80fd\u4fee\u6539\u3002",
    "settings.account.role": "\u89d2\u8272",
    "settings.account.roleAdmin": "\u7ba1\u7406\u5458",
    "settings.account.roleUser": "\u7528\u6237",
    "settings.account.changePasswordTitle": "\u4fee\u6539\u5bc6\u7801",
    "settings.account.currentPassword": "\u5f53\u524d\u5bc6\u7801",
    "settings.account.newPassword": "\u65b0\u5bc6\u7801",
    "settings.account.changePassword": "\u4fee\u6539\u5bc6\u7801",
    "settings.account.changePasswordBusy": "\u4fee\u6539\u4e2d\u2026",
    "settings.account.passwordChanged": "\u5bc6\u7801\u5df2\u4fee\u6539\u3002\u8be5\u8d26\u6237\u7684\u5176\u4ed6\u4f1a\u8bdd\u5df2\u6ce8\u9500\u3002",
    "settings.account.passwordChangeError": "\u4fee\u6539\u5bc6\u7801\u5931\u8d25",
    "settings.account.save": "\u4fdd\u5b58\u8d26\u6237",
    "settings.account.saveBusy": "\u4fdd\u5b58\u4e2d\u2026",
    "settings.account.savedMessage": "\u8d26\u6237\u5df2\u4fdd\u5b58\uff0c\u73b0\u6709\u4f1a\u8bdd\u5df2\u6e05\u9664\u3002",
    "settings.account.saveError": "\u4fdd\u5b58\u8d26\u6237\u5931\u8d25",
    "settings.account.passwordHelp": "\u81f3\u5c11 8 \u4e2a\u5b57\u7b26\u3002",

    "settings.users.eyebrow": "\u7528\u6237\u7ba1\u7406",
    "settings.users.title": "\u7528\u6237",
    "settings.users.description": "\u6dfb\u52a0\u6216\u5220\u9664\u7528\u6237\u3002\u6bcf\u4e2a\u7528\u6237\u62e5\u6709\u72ec\u7acb\u7684\u6587\u6863\u5e93\u548c AI \u63d0\u4f9b\u65b9\u914d\u7f6e\u3002",
    "settings.users.createTitle": "\u6dfb\u52a0\u7528\u6237",
    "settings.users.username": "\u7528\u6237\u540d",
    "settings.users.usernamePlaceholder": "alice",
    "settings.users.password": "\u521d\u59cb\u5bc6\u7801",
    "settings.users.create": "\u6dfb\u52a0\u7528\u6237",
    "settings.users.createBusy": "\u6dfb\u52a0\u4e2d\u2026",
    "settings.users.createdMessage": "\u5df2\u6dfb\u52a0\u7528\u6237\u201c{name}\u201d\u3002\u8bf7\u5b89\u5168\u5730\u544a\u77e5\u5bf9\u65b9\u5bc6\u7801\u3002",
    "settings.users.createError": "\u6dfb\u52a0\u7528\u6237\u5931\u8d25",
    "settings.users.listTitle": "\u73b0\u6709\u7528\u6237",
    "settings.users.loading": "\u52a0\u8f7d\u4e2d\u2026",
    "settings.users.loadError": "\u52a0\u8f7d\u7528\u6237\u5217\u8868\u5931\u8d25",
    "settings.users.empty": "\u8fd8\u6ca1\u6709\u7528\u6237\u3002",
    "settings.users.roleAdmin": "\u7ba1\u7406\u5458",
    "settings.users.roleUser": "\u7528\u6237",
    "settings.users.vault": "\u6587\u6863\u5e93\uff1a{status}",
    "settings.users.vaultConfigured": "\u5df2\u914d\u7f6e",
    "settings.users.vaultMissing": "\u672a\u914d\u7f6e",
    "settings.users.delete": "\u5220\u9664\u2026",
    "settings.users.confirmDelete": "\u786e\u8ba4\u5220\u9664",
    "settings.users.cancel": "\u53d6\u6d88",
    "settings.users.deleteBusy": "\u5220\u9664\u4e2d\u2026",
    "settings.users.deletedMessage": "\u5df2\u5220\u9664\u7528\u6237\u201c{name}\u201d\u3002",
    "settings.users.deleteError": "\u5220\u9664\u7528\u6237\u5931\u8d25",
    "settings.users.deleteCaveat": "\u5220\u9664\u7528\u6237\u4f1a\u6e05\u7406\u8be5\u7528\u6237\u7684 RAG \u7d22\u5f15\u548c\u5143\u6570\u636e\u7f13\u5b58\uff0c\u4f46\u4e0d\u4f1a\u52a8\u78c1\u76d8\u4e0a\u7684 Markdown \u6587\u6863\u5e93\u3002",

    "settings.vault.eyebrow": "\u6587\u6863",
    "settings.vault.title": "Lumynn \u6587\u6863\u5e93",
    "settings.vault.description": "\u9009\u62e9 Lumynn \u4f7f\u7528\u7684\u672c\u5730\u6587\u6863\u5e93\u8def\u5f84\u3002",
    "settings.vault.path": "\u6587\u6863\u5e93\u8def\u5f84",
    "settings.vault.allowPlain": "\u5141\u8bb8\u4f7f\u7528\u666e\u901a Markdown \u6587\u4ef6\u5939",
    "settings.vault.save": "\u4fdd\u5b58\u6587\u6863\u5e93",
    "settings.vault.saveBusy": "\u4fdd\u5b58\u4e2d\u2026",
    "settings.vault.savedMessage": "\u6587\u6863\u5e93\u8bbe\u7f6e\u5df2\u4fdd\u5b58",
    "settings.vault.saveError": "\u4fdd\u5b58\u6587\u6863\u5e93\u5931\u8d25",

    "settings.https.eyebrow": "\u4f20\u8f93",
    "settings.https.title": "HTTPS \u8bc1\u4e66",
    "settings.https.description": "\u7c98\u8d34\u6216\u5bfc\u5165 PEM \u8bc1\u4e66\u4e0e\u79c1\u94a5\u6587\u4ef6\u3002\u4fdd\u5b58\u540e\u9700\u91cd\u542f\u670d\u52a1\u5668\u751f\u6548\u3002",
    "settings.https.enable": "\u91cd\u542f\u540e\u542f\u7528 HTTPS",
    "settings.https.statusConfigured": "\u5df2\u914d\u7f6e",
    "settings.https.statusNot": "\u672a\u914d\u7f6e",
    "settings.https.statusLine": "\u8bc1\u4e66\uff1a{cert} \u00b7 \u79c1\u94a5\uff1a{key}",
    "settings.https.cert": "\u8bc1\u4e66 PEM",
    "settings.https.certPlaceholder": "\u7c98\u8d34 -----BEGIN CERTIFICATE----- \u2026 \u7559\u7a7a\u5219\u4fdd\u7559\u73b0\u6709\u8bc1\u4e66\u3002",
    "settings.https.importCert": "\u5bfc\u5165\u8bc1\u4e66\u6587\u4ef6",
    "settings.https.privateKey": "\u79c1\u94a5 PEM",
    "settings.https.privateKeyPlaceholder": "\u7c98\u8d34 -----BEGIN PRIVATE KEY----- \u2026 \u7559\u7a7a\u5219\u4fdd\u7559\u73b0\u6709\u79c1\u94a5\u3002",
    "settings.https.importKey": "\u5bfc\u5165\u79c1\u94a5\u6587\u4ef6",
    "settings.https.save": "\u4fdd\u5b58 HTTPS \u8bbe\u7f6e",
    "settings.https.saveBusy": "\u4fdd\u5b58\u4e2d\u2026",
    "settings.https.savedMessage": "HTTPS \u8bbe\u7f6e\u5df2\u4fdd\u5b58\uff0c\u91cd\u542f\u670d\u52a1\u5668\u540e\u751f\u6548\u3002",
    "settings.https.saveError": "\u4fdd\u5b58 HTTPS \u8bbe\u7f6e\u5931\u8d25",

    "settings.providers.embeddingEyebrow": "\u5d4c\u5165",
    "settings.providers.embeddingTitle": "\u5d4c\u5165\u63d0\u4f9b\u65b9",
    "settings.providers.qaEyebrow": "\u5bf9\u8bdd",
    "settings.providers.qaTitle": "\u95ee\u7b54\u63d0\u4f9b\u65b9",
    "settings.providers.save": "\u4fdd\u5b58 AI \u63d0\u4f9b\u65b9",
    "settings.providers.saveBusy": "\u4fdd\u5b58\u4e2d\u2026",
    "settings.providers.savedMessage": "RAG \u8bbe\u7f6e\u5df2\u4fdd\u5b58",
    "settings.providers.saveError": "\u4fdd\u5b58 RAG \u8bbe\u7f6e\u5931\u8d25",
    "settings.providers.testEmbedding": "\u6d4b\u8bd5\u5d4c\u5165",
    "settings.providers.testEmbeddingBusy": "\u6d4b\u8bd5\u4e2d\u2026",
    "settings.providers.testQa": "\u6d4b\u8bd5\u95ee\u7b54",
    "settings.providers.testQaBusy": "\u6d4b\u8bd5\u4e2d\u2026",
    "settings.providers.testFailed": "\u6d4b\u8bd5\u5931\u8d25",
    "settings.providers.provider": "\u63d0\u4f9b\u65b9",
    "settings.providers.providerDisabled": "\u5df2\u7981\u7528",
    "settings.providers.providerOpenAi": "OpenAI \u517c\u5bb9",
    "settings.providers.apiMode": "API \u6a21\u5f0f",
    "settings.providers.apiModeEmbeddings": "Embeddings API\uff08/embeddings\uff09",
    "settings.providers.apiModeChat": "Chat Completions API\uff08/chat/completions\uff09",
    "settings.providers.apiModeResponses": "Responses API\uff08/responses\uff09",
    "settings.providers.apiModeCustomEmbedding": "\u81ea\u5b9a\u4e49\u5d4c\u5165\u517c\u5bb9\u8def\u5f84",
    "settings.providers.apiModeCustomChat": "\u81ea\u5b9a\u4e49\u5bf9\u8bdd\u517c\u5bb9\u8def\u5f84",
    "settings.providers.baseUrl": "\u57fa\u7840 URL",
    "settings.providers.endpointPath": "\u63a5\u53e3\u8def\u5f84",
    "settings.providers.reasoning": "\u63a8\u7406\u6a21\u5f0f",
    "settings.providers.reasoningDisabled": "\u7981\u7528\u63a8\u7406 / \u601d\u8003",
    "settings.providers.reasoningDefault": "\u63d0\u4f9b\u65b9\u9ed8\u8ba4",
    "settings.providers.reasoningHelp": "\u5bf9\u4e8e\u53ef\u80fd\u5728\u8f93\u51fa\u4ee4\u724c\u4e0a\u8017\u8d39\u8fc7\u591a\u7684\u63a8\u7406\u6a21\u578b\uff0c\u8bf7\u9009\u201c\u7981\u7528\u201d\u3002",
    "settings.providers.model": "\u6a21\u578b",
    "settings.providers.apiKey": "API Key",
    "settings.providers.apiKeyPlaceholder": "\u7559\u7a7a\u5219\u4fdd\u7559\u73b0\u6709\u5bc6\u94a5",

    "settings.ops.eyebrow": "\u68c0\u7d22\u7d22\u5f15",
    "settings.ops.title": "\u6784\u5efa\u7d22\u5f15",
    "settings.ops.description": "\u521b\u5efa Ask AI \u4f7f\u7528\u7684\u6587\u6863\u5e93\u3002\u5efa\u8bae\u5148\u4ee5\u5c11\u91cf\u6837\u672c\u8bd5\u8dd1\uff0c\u7136\u540e\u4f7f\u7528\u589e\u91cf\u7d22\u5f15\u8fdb\u884c\u65e5\u5e38\u66f4\u65b0\u3002",
    "settings.ops.retrievalGroup": "\u68c0\u7d22\u53c2\u6570",
    "settings.ops.indexingGroup": "\u7d22\u5f15\u53c2\u6570",
    "settings.ops.topK": "Top K",
    "settings.ops.chunkSize": "\u5206\u6bb5\u5927\u5c0f",
    "settings.ops.chunkOverlap": "\u5206\u6bb5\u91cd\u53e0",
    "settings.ops.batchSize": "\u5d4c\u5165\u6279\u6b21\u5927\u5c0f",
    "settings.ops.rpm": "\u5d4c\u5165\u8bf7\u6c42 / \u5206\u949f",
    "settings.ops.partitions": "\u5206\u533a\u6570\u91cf",
    "settings.ops.saveIndex": "\u4fdd\u5b58\u7d22\u5f15\u8bbe\u7f6e",
    "settings.ops.saveIndexBusy": "\u4fdd\u5b58\u4e2d\u2026",
    "settings.ops.resetIndex": "\u91cd\u7f6e\u7d22\u5f15\u8bbe\u7f6e",
    "settings.ops.resetIndexBusy": "\u91cd\u7f6e\u4e2d\u2026",
    "settings.ops.resetSaved": "\u5df2\u5c06\u7d22\u5f15\u8bbe\u7f6e\u91cd\u7f6e\u4e3a\u9ed8\u8ba4\u503c\u5e76\u4fdd\u5b58",
    "settings.ops.resetError": "\u91cd\u7f6e\u7d22\u5f15\u8bbe\u7f6e\u5931\u8d25",
    "settings.ops.testIndex": "\u8bd5\u8dd1 20 \u4e2a\u6587\u4ef6\u6837\u672c",
    "settings.ops.testIndexBusy": "\u542f\u52a8\u4e2d\u2026",
    "settings.ops.incremental": "\u5f00\u59cb\u589e\u91cf\u7d22\u5f15",
    "settings.ops.incrementalBusy": "\u542f\u52a8\u4e2d\u2026",
    "settings.ops.full": "\u91cd\u5efa\u5168\u91cf\u7d22\u5f15",
    "settings.ops.fullBusy": "\u542f\u52a8\u4e2d\u2026",
    "settings.ops.startedMessage": "\u5df2\u542f\u52a8 {mode} \u7d22\u5f15\u4efb\u52a1 {id}",
    "settings.ops.startError": "\u542f\u52a8\u7d22\u5f15\u5931\u8d25",
    "settings.ops.stopRequested": "\u5df2\u8bf7\u6c42\u505c\u6b62\u7d22\u5f15\u4efb\u52a1",
    "settings.ops.skipRequested": "\u5df2\u8bf7\u6c42\u8df3\u8fc7\u5f53\u524d\u6587\u4ef6",
    "settings.ops.controlError": "\u66f4\u65b0\u7d22\u5f15\u4efb\u52a1\u5931\u8d25",

    "settings.indexStatus.aria": "RAG \u7d22\u5f15\u72b6\u6001",
    "settings.indexStatus.production": "\u751f\u4ea7\u7d22\u5f15",
    "settings.indexStatus.test": "\u6d4b\u8bd5\u7d22\u5f15",
    "settings.indexStatus.indexed": "\u5df2\u7d22\u5f15",
    "settings.indexStatus.notIndexed": "\u672a\u7d22\u5f15",
    "settings.indexStatus.files": "{count} \u4e2a\u6587\u4ef6",
    "settings.indexStatus.chunks": "{count} \u4e2a\u5206\u6bb5",
    "settings.indexStatus.lastUpdated": "\u6700\u540e\u66f4\u65b0\uff1a{value}",
    "settings.indexStatus.never": "\u4ece\u672a",

    "settings.progress.test": "\u6d4b\u8bd5\u7d22\u5f15",
    "settings.progress.incremental": "\u589e\u91cf\u7d22\u5f15",
    "settings.progress.full": "\u5168\u91cf\u7d22\u5f15",
    "settings.progress.files": "\u6587\u4ef6",
    "settings.progress.chunks": "\u5206\u6bb5",
    "settings.progress.skippedFiles": "\u5df2\u8df3\u8fc7\u6587\u4ef6\uff1a{count}",
    "settings.progress.reusedChunks": "\u590d\u7528\u5206\u6bb5\uff1a{count}",
    "settings.progress.failedChunks": "\u5931\u8d25\u5206\u6bb5\uff1a{count}",
    "settings.progress.currentFile": "\u5f53\u524d\uff1a{name}",
    "settings.progress.skipCurrent": "\u8df3\u8fc7\u5f53\u524d\u6587\u4ef6",
    "settings.progress.skipRequestedShort": "\u5df2\u8bf7\u6c42\u8df3\u8fc7",
    "settings.progress.stop": "\u505c\u6b62\u7d22\u5f15",
    "settings.progress.stopping": "\u505c\u6b62\u4e2d\u2026",
    "settings.progress.elapsed": "{ms}\u00a0\u6beb\u79d2",
    "settings.jobStatus.queued": "\u6392\u961f\u4e2d",
    "settings.jobStatus.running": "\u8fd0\u884c\u4e2d",
    "settings.jobStatus.completed": "\u5df2\u5b8c\u6210",
    "settings.jobStatus.cancelled": "\u5df2\u53d6\u6d88",
    "settings.jobStatus.failed": "\u5df2\u5931\u8d25",
    "settings.message.dismiss": "\u5173\u95ed\u6d88\u606f",

    "settings.export.eyebrow": "\u5bfc\u51fa",
    "settings.export.title": "RAG \u5bfc\u51fa",
    "settings.export.description": "\u5bfc\u51fa\u7684\u914d\u7f6e\u4e0d\u5305\u542b\u771f\u5b9e API \u5bc6\u94a5\u3002",
    "settings.export.aria": "RAG \u914d\u7f6e\u5bfc\u51fa",
    "settings.export.copy": "\u590d\u5236\u5230\u526a\u8d34\u677f",
    "settings.export.copyBusy": "\u590d\u5236\u4e2d\u2026",
    "settings.export.copySuccess": "RAG \u914d\u7f6e\u5df2\u590d\u5236\u5230\u526a\u8d34\u677f",
    "settings.export.copyError": "\u526a\u8d34\u677f\u5199\u5165\u5931\u8d25\uff0c\u8bf7\u624b\u52a8\u590d\u5236\u5bfc\u51fa\u6587\u672c\u3002",
    "settings.export.toFile": "\u5bfc\u51fa\u5230\u6587\u4ef6",
    "settings.export.fileSavedMessage": "RAG \u914d\u7f6e\u5df2\u5bfc\u51fa\u5230\u6587\u4ef6",

    "settings.import.eyebrow": "\u5bfc\u5165",
    "settings.import.title": "RAG \u5bfc\u5165",
    "settings.import.description": "\u4ece\u526a\u8d34\u677f\u7c98\u8d34\u6216\u5bfc\u5165 schemaVersion 1 \u7684 RAG \u914d\u7f6e JSON \u6587\u4ef6\u3002",
    "settings.import.aria": "RAG \u914d\u7f6e\u5bfc\u5165",
    "settings.import.paste": "\u4ece\u526a\u8d34\u677f\u7c98\u8d34",
    "settings.import.pasteBusy": "\u7c98\u8d34\u4e2d\u2026",
    "settings.import.pasteSuccess": "RAG \u914d\u7f6e\u5df2\u4ece\u526a\u8d34\u677f\u7c98\u8d34",
    "settings.import.pasteError": "\u526a\u8d34\u677f\u8bfb\u53d6\u5931\u8d25\uff0c\u8bf7\u624b\u52a8\u7c98\u8d34\u914d\u7f6e\u3002",
    "settings.import.fromFile": "\u4ece\u6587\u4ef6\u5bfc\u5165",
    "settings.import.loadedFile": "\u5df2\u52a0\u8f7d {name}\uff0c\u68c0\u67e5\u540e\u70b9\u51fb\u201c\u5bfc\u5165 RAG \u914d\u7f6e\u201d\u3002",
    "settings.import.submit": "\u5bfc\u5165 RAG \u914d\u7f6e",
    "settings.import.submitBusy": "\u5bfc\u5165\u4e2d\u2026",
    "settings.import.success": "RAG \u914d\u7f6e\u5df2\u5bfc\u5165",
    "settings.import.failed": "\u5bfc\u5165\u5931\u8d25",

    "error.actionFailed": "\u64cd\u4f5c\u5931\u8d25"
  }
} as const satisfies Record<Locale, Record<string, string>>;

export type TKey = keyof typeof dictionaries.en;

function detectInitialLocale(): Locale {
  if (typeof window === "undefined") return "en";
  try {
    const stored = window.localStorage.getItem(STORAGE_KEY);
    if (stored === "en" || stored === "zh") return stored;
  } catch {
    // ignore storage errors
  }
  const langs = typeof navigator !== "undefined" ? navigator.languages ?? [navigator.language] : [];
  for (const lang of langs) {
    if (!lang) continue;
    const lower = lang.toLowerCase();
    if (lower.startsWith("zh")) return "zh";
    if (lower.startsWith("en")) return "en";
  }
  return "en";
}

interface LocaleContextValue {
  locale: Locale;
  setLocale: (next: Locale) => void;
  t: (key: TKey, params?: Record<string, string | number>) => string;
}

const LocaleContext = createContext<LocaleContextValue | null>(null);

export function LocaleProvider(props: { children: ReactNode }) {
  const [locale, setLocaleState] = useState<Locale>(detectInitialLocale);

  useEffect(() => {
    if (typeof document !== "undefined") {
      document.documentElement.lang = locale === "zh" ? "zh-Hans" : "en";
    }
    try {
      window.localStorage.setItem(STORAGE_KEY, locale);
    } catch {
      // ignore storage errors
    }
  }, [locale]);

  const setLocale = useCallback((next: Locale) => {
    setLocaleState(next);
  }, []);

  const t = useCallback(
    (key: TKey, params?: Record<string, string | number>) => {
      const dict = dictionaries[locale];
      const fallback = dictionaries.en;
      const template = (dict[key] ?? fallback[key] ?? key) as string;
      if (!params) return template;
      return template.replace(/\{(\w+)\}/g, (_, name: string) =>
        Object.prototype.hasOwnProperty.call(params, name) ? String(params[name]) : `{${name}}`
      );
    },
    [locale]
  );

  const value = useMemo(() => ({ locale, setLocale, t }), [locale, setLocale, t]);

  return <LocaleContext.Provider value={value}>{props.children}</LocaleContext.Provider>;
}

export function useLocale(): LocaleContextValue {
  const ctx = useContext(LocaleContext);
  if (!ctx) {
    throw new Error("useLocale must be used within <LocaleProvider>");
  }
  return ctx;
}

export function useT() {
  return useLocale().t;
}
