...
export class Muya {
    ...
    insertMarkdownAtSelection(markdown: string) {
        ...
        if (!anchorBlock || !cursor) {
            this.setContent(`${this.getMarkdown()}${markdown}`, true);
            return;
        }

        const start = Math.min(cursor.start?.offset ?? 0, cursor.end?.offset ?? cursor.start?.offset ?? 0);
        const end = Math.max(cursor.start?.offset ?? start, cursor.end?.offset ?? start);
        const text = anchorBlock.text ?? '';
        anchorBlock.text = `${text.slice(0, start)}${markdown}${text.slice(end)}`;
        anchorBlock.update();
        anchorBlock.setCursor(start + markdown.length, start + markdown.length, true);
    }
