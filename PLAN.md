...
- Quick note capture: a one-tap shortcut available from the desktop topbar (a "Quick note" pill, also bound to `Cmd/Ctrl + Shift + N`) and from a `+` icon in the mobile app bar. Opens a focused capture sheet (bottom sheet on mobile, centered dialog on desktop) with just a textarea and a Save button. Save flow needs no folder picker and no file name dialog: the file name is derived from the first Markdown heading, then the first non-empty line, then a localized timestamp like `Quick note 2026-05-01 12-30.md`. Files land in a `Quick notes/` folder and use the existing collision suffix logic so duplicate names never block the user. Saved notes immediately open in the editor and surface an Undo toast that, when invoked, deletes the just-created file.
