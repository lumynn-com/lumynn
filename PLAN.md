...
- Rename, create, edit, preview, save, and delete document actions. Pasting an image (screenshot or copied file) into the editor textarea uploads the bytes to a hardcoded `attachments/` folder inside the library and inserts an Obsidian-style `![[attachments/...]]` reference at the cursor; the upload runs in the background while a placeholder marker is shown inline, and on success/failure the placeholder is swapped for the real wiki-link or an HTML comment with the error. The document tree tracks an explicit "selected folder" (highlighted in the tree) that updates when the user taps a folder row or opens a file (in which case the file's parent becomes the implicit selected folder). The New Note dialog pre-fills the path from the selected folder so a new note lands where the user is looking; falls back to library root when no folder has been picked.
- Library search modal that uses `obsidian-cli` first and falls back to filesystem search.
- Library search results show clickable file names only, truncated to one line when too long, and the result list scrolls inside the modal.
...
- Hybrid retrieval combines vector search, indexed keyword search, metadata boosts, and limited live-library fallback.
...
- Installable Progressive Web App on Android Chrome and iOS Safari: ships a hand-rolled `public/sw.js` service worker that pre-caches the SPA shell on install, drops older cache versions on activate, and uses three fetch strategies on same-origin GETs (network-only for `/api/*` so authenticated library data is never cached, network-first for navigations with the cached shell as offline fallback, stale-while-revalidate for built JS/CSS/icons). The worker is registered from `main.tsx` only in production secure contexts. The manifest declares `id`, `lang`, `dir`, `categories`, and marks the 192/512 icons as `purpose: "any maskable"` so Android's adaptive icon mask renders cleanly.
...
- Native-feeling mobile interactions on small screens: a sticky top segmented control (right under the global topbar) switches between Library, Editor, and Ask while keeping each pane mounted so state is preserved; selecting a file in the tree or a citation in Ask automatically switches to the Editor; pulling the document tree down past a threshold opens the library search modal with a "Pull to search / Release to search" indicator; and editor tabs can be swiped left to close. Desktop behavior is unchanged.
...
- Settings page interactions reflect request state: every Save / Test / Start / Stop / Copy / Paste / Import action shows a localized loading label (Saving\u2026, Testing\u2026, Starting\u2026, Stopping\u2026, Copying\u2026, Pasting\u2026, Importing\u2026) with `aria-busy` and the button disabled while the request is in flight, tracked through a single `runBusy` helper that supports concurrent independent actions. Account, library, HTTPS, and provider inputs carry stable `name`/`autoComplete`/`spellCheck` attributes for password manager and field-recall behavior.
...
- Mobile library list shows a circular "back to top" button after scrolling more than ~220px; tapping it smooth-scrolls the tree back and fires a subtle haptic.
...
- Mobile chrome refactor: each pane now exposes a single thin sticky **section header** (title + 1\u20133 round icon buttons) instead of the desktop-style panel header + full-width text-button toolbar that used to dominate the screen. Library header has Search / New / Sort icons; Editor header has an Edit\u2194Preview toggle + an overflow menu containing Rename and Delete; Ask drops the eyebrow/title/description entirely so the question input is the first thing the user sees. Sort and order options live in a bottom **action sheet** that slides in from the bottom safe area. The Save FAB still floats bottom-right when the editor has unsaved changes. Topbar, segmented control, and pane padding all tightened so the chrome is under \u2248 110px and the content fills the rest. Desktop layout, panel headers, and original toolbars are unchanged.
...
- Editor-first mobile model: the desktop topbar, segmented control, and per-pane section headers are all replaced on mobile by a **single sticky app bar** at the very top: hamburger on the left (opens the library as a left **drawer**), file name + path centered, and a more (\u22ef) icon on the right that opens a **command sheet**. Editor is the always-visible main view. Library slides in from the left edge over a tap-to-close backdrop; Ask slides up from the bottom. The command sheet groups every remaining action: Edit/Preview toggle, Save, Rename, Delete, Ask AI, Indexing, Settings, theme toggle, language toggle, and Log Out. Indexing and Settings on mobile expose a back arrow at the top to return to the workspace. Total chrome above content collapses to \u2248 50px (the app bar) on mobile.
...
- Edge-swipe between workspace panes on mobile: a horizontal swipe that starts within ~26px of the left or right screen edge cycles between Library, Editor, and Ask without needing to reach the bottom tab bar.
- Per-tab edit/preview mode: each open tab tracks its own mode instead of one global setting. Opening an existing file now always lands in preview mode so the reader sees the rendered note first; drafts (Quick Note) and freshly created blank notes open in edit mode because a blank preview is useless. Switching modes via the toolbar pill, the command sheet, or the mode-aware Save/Edit FAB only affects the active tab — flipping tabs restores whatever mode each one was last in. Tab modes are session state (not persisted across reloads); the old `owd_editor_mode` localStorage key was retired.
- Save-then-preview flow: a successful manual save (toolbar button or mobile Save FAB) automatically switches the active tab to preview mode so the user immediately sees the rendered output. The mobile FAB is a single contextual button with three states and no disabled fallback: in **preview** it's **Edit** (secondary), in **edit + dirty** it's **Save** (primary, switches to preview after success), and in **edit + clean** it's **Preview** (secondary) so the user can flip back to the rendered view without going through the toolbar. Auto-save does not switch modes.
- Auto-save every 5 seconds for the active document. Conditions: a real on-disk file is open (not a draft tab), the editor draft is non-empty, the draft differs from the last saved content, and no save is currently in flight. Auto-save is silent (no status pill, no haptic, no preview switch, no document-list refresh) so it stays out of the user's way; manual save still does the full flow with sort-order refresh and mode switch.
- Muya WYSIWYG editor integration: the editor panel now embeds the local `../muya` editor instead of the old textarea surface while keeping the app's font stack, Obsidian image-embed round-tripping, paste-to-attachment uploads, and per-tab edit/preview flow. Task-list parsing/rendering was hardened so loose/empty task-list items do not leak raw `[ ]` marker text into the editor or preview, and empty `- [ ]` items still render as normal task checkboxes in preview. App-level `input`/`button`/`select`/`textarea` CSS is explicitly scoped away from Muya editor and floating-toolbar DOM so Muya controls keep their own sizing, hit areas, and visual rules. Auto-save no longer replaces the live editor draft with server-returned content, and Muya no longer runs in-place `setContent()` for external value updates after mount; file switches still remount by tab path, but rapid typing during auto-save no longer loses caret/scroll position or jumps to the document top. On touch devices, pressing Muya's paragraph front button now first blurs the editor, clears any editor selection, and focuses the body without scrolling, which dismisses the native text-selection/context menu before Muya opens its own block toolbar while leaving normal editor text selection and system copy/paste menus intact.
- Long-lived authentication ("remember me" by default): the session cookie now lives 90 days and the server-side record is refreshed (sliding window) every time the cookie is more than half a TTL old, so an actively used session never lapses. The auth-status check (`GET /api/auth/me`) participates in the refresh, so simply opening the app warms the session forward. Fully dormant sessions still expire after 90 days for security.
- Multi-user support with full per-user isolation. Admins can add/delete users from a new **Settings → Users** page; each user owns their own library path, AI provider configuration (embedding + Q&A keys, model, etc.), document metadata cache, and RAG vector index. The data model bumps `app-data.json` to schema v2 with a `users[]` array and a `globalSettings` object (HTTPS only); the legacy single-admin layout is migrated in-place on first boot. Library scoping: every documents/RAG route reads the existing internal `request.user.vault.path` field (decorated by the `requireAuth` preHandler), so users literally cannot see each other's notes; the in-memory tree-listing cache is keyed by absolute library root so two users with different libraries never share entries. RAG: vector-index files live under `data/vector-index/<sha256(username).slice(0,16)>/<production|test>.{jsonl,f32,manifest.json,files.json}`, and a global job map enforces one running indexing job per user (different users index in parallel). Self-service: every user can change their own password (verifies current, then invalidates other sessions for that account); usernames are immutable. Admin restrictions: the spec says no backdoor, so admins **cannot** read another user's library, edit another user's RAG config, or reset another user's password. CLI escape hatch: `./server.sh reset-password <username> <new-password>` writes a new bcrypt hash directly into the JSON store and invalidates that user's sessions, for the (rare) case that someone forgets their password and there is no admin who can help.
- Site-wide flat UI pass: replaced gradients, blurred backdrops, decorative shadows, and nested toolbar cards with a single hairline divider language, calmer flat surfaces, and one set of radii (16 px for modals / 8 px for icon buttons / 6 px for compact tab-bar buttons). Stripped panel-style chrome from Library, Editor, and Q&A — replaced with thin inline meta lines or removed entirely when redundant, and folded rarely-used actions into overflow menus. The editor toolbar collapsed into the tab strip itself: a single thin band carries the tabs, the live status pill (with `aria-live="polite"` and a `title` for the full message when truncated), an Edit\u2194Preview toggle, the focus-mode button, and a "More" trigger that opens the desktop command sheet. The desktop top bar was deleted; its workspace navigation, theme toggle, language toggle, and logout were moved into the More command sheet so desktop and mobile share one entry point. Save is also bound to `Cmd/Ctrl + S` from the editor textarea. Light-theme tokens were restructured at the same time (stronger border opacities, slightly darker `--md-surface-2/3`, explicit hover/active tints on `.icon-button` / `.tree-row` / tab strip / citations) so flat surfaces stay legible in bright environments without leaning on shadows; the `.primary` button rule was hardened with `!important` on `background-color` / `color` to win a long-standing CSS specificity fight that left primary buttons transparent in light mode.
- Collapsible Library and Q&A side panels (desktop \u2265 861 px). Either pane can collapse to a 44 px rail with a single expand button + a passive icon hint (`MenuIcon` for Library, `AskIcon` for Q&A); the editor expands into the freed columns via dynamic `grid-template-columns` driven off `data-vault-collapsed` / `data-qa-collapsed` data attributes on the workspace. State persists in `localStorage` (`owd_vault_collapsed`, `owd_qa_collapsed`) so the layout is stable across reloads. Both rails carry the `inert` attribute while the pane is expanded so the expand button is removed from the keyboard tab order and the accessibility tree until needed (modern Chrome/Safari/Firefox honor it natively). The library toolbar's "Search" button is icon-only with an accent tint so the row never truncates its localized label across English / Chinese; the full label is preserved via `aria-label` and `title`.
- Distraction-free focus mode (desktop): toggling the focus button hides every chrome surface \u2014 workspace gutters, tab bar, hairlines, both side panels \u2014 so the editor / preview fills the entire viewport edge to edge. The single dismiss affordance is a compact fixed pill in the upper-right that pairs a minimize glyph with an `Esc` `<kbd>` hint; clicking it or pressing `Escape` returns to the regular layout. The pill is positioned `2.5rem` inside the right edge so it never overlaps the textarea's overlay scrollbar, animates only `background-color` / `border-color` / `color` / `box-shadow` (no `transition: all`), and respects `prefers-reduced-motion`.
- Indexing and Settings on desktop now open as overlay modals on top of the workspace instead of replacing the entire main view, so dismissing the dialog drops the user straight back into the editor with the tree / tabs / draft state intact. The dialog reuses the existing `<SettingsView>` component but suppresses its own back-bar / hero header so the modal owns the chrome (eyebrow + title + a single \u00d7 close button, with `data-autofocus` so keyboard users land inside on open). It supports focus trapping (`Tab` / `Shift-Tab` cycle between the first and last focusable element of the dialog), `<body>` scroll lock while open, focus restoration to the trigger on close, outside-click and `Escape` dismiss, `overscroll-behavior: contain` on the body, and a sticky header hairline + soft drop shadow that fade in once the body scrolls more than 1 px. Mobile keeps the legacy full-view switch (a small-screen modal would be unusable), and the "Ask" command in the desktop command sheet is also hidden on desktop because the Q&A panel is always rendered (or visible as a 44 px collapsed rail). The legacy view-switch mounts in `App.tsx` are now gated behind `isMobile`, and the `view` state snaps back to `"workspace"` if the viewport flips from mobile to desktop while a legacy view was active. All modals (`.settings-modal`, `.search-modal`, `.prompt-modal`) share a single 16 px radius and the same `owd-modal-in` 180 ms scale-fade entrance keyframe gated by `prefers-reduced-motion`.
- Vercel Web Interface Guidelines pass on copy and translation: every English heading, dialog title, menu item, and button label was passed through Chicago Title Case ("Move To\u2026", "New Folder Here", "Save Account", "Ask Your Library", "Confirm Delete", "Save HTTPS Settings", etc.); status messages, form field labels, and helper text were intentionally kept in sentence case so long labels do not feel shouty. Every place that surfaces a user-generated identifier \u2014 library-tree folder rows, the folder-picker rows used by New Note / Rename / Move dialogs, citation titles + paths, tab paths, search results, the undo toast, the active-doc breadcrumb on mobile, the users-list username \u2014 is wrapped with `translate="no"` so Chrome / Safari auto-translate cannot silently rename folders or paths so they no longer match the file they represent. A new `modal.close` i18n key replaces the misleading "Cancel" aria-label that the settings-modal close button used to inherit.
- Performance touches: every `.tree-row` in the library tree carries `content-visibility: auto; contain-intrinsic-size: auto 34px;` so the browser skips layout, paint, and style work for off-screen rows until they scroll near the viewport, with the `auto` keyword caching the actual rendered size so the scrollbar stays accurate even on huge libraries; the loading-spinner placeholder is exempted so spinners keep animating while a folder is fetching. Markdown headings inside `.preview-surface article` and inside the `.qa-answer` panel both carry `scroll-margin-top: 28px` so deep-linked anchor jumps (TOC clicks, backlinks, citation jumps) land with breathing room above the heading rather than flush against the top edge of the scroll container. Initial workspace refresh now uses a lightweight `/api/documents/count` endpoint for the library file count instead of calling full `/api/documents`; the heavier metadata listing remains available for search/RAG/on-demand use. On cold start, the root tree still loads immediately, but document-count refresh, recursive folder warmup, and edit-mode preview pre-rendering are deferred until the Muya editor has initialized and painted once, so first input readiness does not compete with library walking or background tree fetches. Muya editor startup was further trimmed by direct-importing the editor modules, splitting non-core UI plugins (table/image/emoji/code-language/preview float tools) into an idle-loaded `muyaAdvancedPlugins` chunk after editor ready, and switching the math renderer's `mhchem` import to KaTeX's ESM export so the bundle no longer includes both `katex.mjs` and `katex.js`.
- Golden performance change: the production Fastify server now precompresses built static assets on startup and serves Brotli/Gzip variants through `@fastify/static` with `preCompressed: true`. Fingerprinted `/assets/*` files use `Cache-Control: public, max-age=31536000, immutable`, while `index.html`, `sw.js`, and `manifest.webmanifest` stay `no-cache` so deployments update safely. This directly reduces remote first-load transfer cost without changing UI behavior: the Muya editor chunk drops from about 1.0 MB to about 289 KB over Brotli, the main app JS from about 338 KB to about 90 KB, and the main CSS from about 179 KB to about 27 KB. Compressed responses also include `Vary: Origin, Accept-Encoding` so browser/CDN caches do not mix compressed and uncompressed variants.
- Redesign branch stabilization: tree refreshes now pass explicit sort/order through lazy folder loading so a sort click cannot reuse stale state; background folder warmup is bounded to the first 40 folders so large libraries do not trigger whole-tree fetch storms; preview requests are abortable and sequence-guarded so stale responses cannot overwrite the active file; unused tree/modal code was removed; and folder-picker expand/collapse labels are localized.
- Guarded visual polish layer: a final CSS section improves workspace surface separation, light-mode contrast, empty/loading states, and non-menu hover/press feedback while deliberately preserving the existing font stack, tab strip, action sheets/context menus, and Markdown preview typography/content styling.
- Final minimalist visual tuning: a late token-only theme layer keeps the light theme warm sand instead of stark white and makes dark mode read as near-black with a subtle purple cast in surfaces, dividers, muted text, and editor focus state. The editor textarea no longer shows a harsh black/white browser focus outline; it signals "ready to type" through a slightly lighter focused surface while other controls retain accessible focus rings. A flat radius normalization layer removes large soft corners from structural panels, tree/search rows, tabs, and menu rows, leaving only small 4 px radii on controls and 8 px on desktop overlays. Mobile Preview/Edit/Save FABs and the desktop focus-mode exit button share a softened utility treatment: translucent surface, light outline, muted text/icon color, and no shadow.
...
- Treat Markdown files as plain-text source files, compatible with Obsidian-style Markdown conventions.
- Support a configurable library path from the web UI.
...
- On mobile, provide native-feeling navigation between the workspace's three panes via a sticky top segmented control (Library / Editor / Ask) right under the global topbar, preserve pane state across switches by hiding instead of unmounting, auto-switch to the Editor when a file or AI citation is opened, support pulling the document tree down to open library search, and support swipe-left-to-close on editor tabs.
- Include README dependency/setup guidance for macOS and Debian.

### Must-Have Document UX

- List Markdown documents from the library.
...
- Provide a library search modal; prefer `obsidian-cli search` when available and fall back to filesystem search.
...
- Library settings must validate path safety and enforce allowed roots.
...
- Retrieval should be hybrid:
  ...
  - Metadata boosts.
  - Library-wide keyword fallback if indexed retrieval fails.
...
- Plain-text Markdown library browsing, creation, editing, preview, and deletion.
- Filesystem-first library operations with path validation.
...
- `obsidian-cli search` integration for library search when available, with filesystem fallback.
...
- Settings page organized by functional area:
  ...
  - RAG operations.
  - RAG import/export.
- Library root enforcement through `ALLOWED_VAULT_ROOTS`.
- Production secret checks for session/encryption/library-root settings.
...
- Classic three-column Lumynn workspace on desktop:
  ...
  - The `obsidian-workspace` `<main>` exposes `data-vault-collapsed` / `data-qa-collapsed` so CSS Grid can swap between expanded / collapsed column widths.
- Single overflow command sheet for global actions (workspace nav to Indexing / Settings, theme toggle, language toggle, log out, plus the editor's secondary actions when a doc is open). On desktop the sheet is opened from the editor tab bar's "More" trigger; on mobile it is opened from the single sticky app bar's `\u22ef` icon. The desktop topbar has been removed entirely so desktop and mobile share one entry point. "Ask" is hidden in the sheet on desktop since the Q&A panel is always present (or visible as a collapsed rail).
- Settings and Indexing on desktop open as overlay modals owned by the workspace, with focus trap, `<body>` scroll lock, focus restoration on close, outside-click + Escape dismiss, sticky header hairline that fades in once the body scrolls, and a 180 ms scale-fade entrance gated by `prefers-reduced-motion`. Mobile keeps the legacy full-view switch with a back arrow.
- Distraction-free focus mode on desktop: hides workspace gutters, tab bar, hairlines, and both side panels so the editor fills the viewport edge to edge; exit pill in the upper-right combines a minimize glyph + an `Esc` `<kbd>` hint, and `Escape` also exits.
...
- Storage:
  - Markdown documents in the configured library.
...
- Rendering:
  ...
  - Library media assets are served through the backend for safe relative image and Obsidian embed rendering.

## Remaining Hardening

- Encrypt provider API keys at rest using `APP_ENCRYPTION_KEY`.
- Add automated backend/frontend tests for auth, library path validation, RAG indexing, and provider modes.
...
- Consider a production-grade vector database if the compact snapshot store becomes too slow for large libraries.
