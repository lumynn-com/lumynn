...
- Save-then-preview flow: a successful manual save (toolbar button or mobile Save FAB) automatically switches the editor to preview mode so the user immediately sees the rendered output. On mobile the Save FAB is mode-aware: in edit mode it shows **Save** (primary, disabled when clean), in preview mode it shows **Edit** (secondary) so the user can flip back into editing with one tap. Auto-save does not switch modes.
